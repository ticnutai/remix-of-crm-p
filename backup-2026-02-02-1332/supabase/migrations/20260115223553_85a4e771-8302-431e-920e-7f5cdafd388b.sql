...
-- Policies (using DROP IF EXISTS pattern for idempotency)
DROP POLICY IF EXISTS "Anyone can read templates" ON public.stage_templates;
CREATE POLICY "Anyone can read templates" ON public.stage_templates FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated users can create templates" ON public.stage_templates;
...
DROP POLICY IF EXISTS "Authenticated users can update templates" ON public.stage_templates;
CREATE POLICY "Authenticated users can update templates" ON public.stage_templates FOR UPDATE USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authenticated users can delete templates" ON public.stage_templates;
CREATE POLICY "Authenticated users can delete templates" ON public.stage_templates FOR DELETE USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Anyone can read template stages" ON public.stage_template_stages;
CREATE POLICY "Anyone can read template stages" ON public.stage_template_stages FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage template stages" ON public.stage_template_stages;
...
DROP POLICY IF EXISTS "Anyone can read template tasks" ON public.stage_template_tasks;
CREATE POLICY "Anyone can read template tasks" ON public.stage_template_tasks FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage template tasks" ON public.stage_template_tasks;
...
-- Trigger
CREATE OR REPLACE FUNCTION update_stage_templates_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = now(); RETURN NEW; END; $$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS update_stage_templates_timestamp ON public.stage_templates;
CREATE TRIGGER update_stage_templates_timestamp BEFORE UPDATE ON public.stage_templates FOR EACH ROW EXECUTE FUNCTION update_stage_templates_updated_at();
