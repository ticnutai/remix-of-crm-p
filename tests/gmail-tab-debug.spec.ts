...
async function loginIfNeeded(page: Page) {
  if (page.url().includes("/auth") || page.url().includes("login")) {
    ...
    await page.fill(
      'input[type="password"]',
      process.env.TEST_PASSWORD || "543211",
    );
...
test("ChatFilePicker Gmail Tab Debug", async ({ page }) => {
  ...
  await page.goto("http://localhost:8080/smart-tools", {
    waitUntil: "domcontentloaded",
    timeout: 20000,
  });
  await page.waitForTimeout(2000);
  ...
  console.log(
    ...
    rlsErrors.length === 0
      ? "✅ none"
      : rlsErrors.map((e) => e.text).join(", "),
  );

  // Click messenger tab
  const messengerTab = page
    .locator('[value="messenger"]')
    .or(page.locator('button:has-text("שיחות")'));
  ...
  await page.screenshot({
    path: "test-results/02-messenger.png",
    fullPage: true,
  });
  ...
  console.log(
    "\n== Conversations:",
    noConvs > 0 ? "NONE - creating..." : "exist",
  );

  if (noConvs > 0) {
    // Click "חדש" button to create a conversation
    const newBtn = page
      .locator('button:has-text("חדש")')
    ...
    if ((await newBtn.count()) > 0) {
      ...
      await page.screenshot({
        path: "test-results/03-new-conv-dialog.png",
        fullPage: true,
      });
      ...
      const groupTypeBtn = page.locator(
        '[role="dialog"] button:has-text("קבוצה")',
      );
      ...
      const submitBtn = page
        .locator(
          '[role="dialog"] button:has-text("צור"), [role="dialog"] button:has-text("יצירה"), [role="dialog"] button[type="submit"]',
        )
        .last();
      ...
      await page.screenshot({
        path: "test-results/03b-after-create.png",
        fullPage: true,
      });
    }
  }
  ...
  await page.screenshot({
    path: "test-results/04-conv-open.png",
    fullPage: true,
  });
  ...
  if (!paperclipClicked)
    console.log(
      "⚠️  Paperclip not found by content — printing SVG btn titles:",
      (
        await Promise.all(
          allSvgBtns
            .slice(0, 30)
            .map((b) => b.getAttribute("title").catch(() => "")),
        )
      ).join(" | "),
    );

  await page.screenshot({
    path: "test-results/05-after-paperclip.png",
    fullPage: true,
  });
  ...
  if (!dialogOpen) {
    for (let i = 0; i < allSvgBtns.length && !dialogOpen; i++) {
      ...
      if ((await page.locator('[role="dialog"]').count()) > 0) {
        const t = await page
          .locator(
            '[role="dialog"] [class*="Title"], [role="dialog"] h2, [role="dialog"] h3',
          )
          .first()
          .textContent()
          .catch(() => "");
  ...
  if (dialogOpen) {
    ...
    if ((await gmailTab.count()) > 0) {
      ...
      await page.screenshot({
        path: "test-results/07-gmail.png",
        fullPage: true,
      });

      const pLogs = logs.filter(
        (l) =>
          l.text.includes("[ChatFilePicker]") || l.text.includes("[GmailTab]"),
      );
      ...
      const spin = await page
        ...
        .count();
      console.log(
        `Spinner: ${spin > 0 ? "🔄 spinning (might be stuck)" : "✅ none"}`,
      );

      // Tab states after Gmail
      for (let i = 0; i < tc; i++) {
        console.log(
          `  Tab[${i}] now: "${((await tabs.nth(i).textContent()) || "").trim()}" (${await tabs.nth(i).getAttribute("data-state")})`,
        );
      }
      ...
      if ((await uploadTab.count()) > 0) {
        ...
        await page.screenshot({
          path: "test-results/08-upload-after-gmail.png",
          fullPage: true,
        });
      }
    } else {
      console.log("⚠️  Gmail tab not found");
      const dHTML = await page
        .locator('[role="dialog"]')
        .innerHTML()
        .catch(() => "");
    ...
    if (!convOpened)
      console.log(
        "💡 Reason: no conversation was opened — paperclip only appears in open chats",
      );
  }

  // Summary
  console.log("\n== SUMMARY ==");
  const allPickerLogs = logs.filter(
    (l) => l.text.includes("[ChatFilePicker]") || l.text.includes("[GmailTab]"),
  );
  console.log(
    `[ChatFilePicker] logs: ${allPickerLogs.length} ${allPickerLogs.length === 0 ? "⚠️ (no debug code loaded?)" : "✅"}`,
  );
  console.log(
    `Console errors: ${logs.filter((l) => l.type === "error").length}`,
  );
