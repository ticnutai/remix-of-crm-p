...
test("Console audit — capture ALL output on key pages", async ({ page }) => {
  ...
  const allLogs: Array<{ type: string; text: string; page: string; time: number }> = [];
  let currentPage = "init";
  const startTime = Date.now();

  page.on("console", (msg) => {
    const text = msg.text();
    if (text.includes("[vite]") || text.includes("HMR") || text.includes("[Vue warn]")) return;
    allLogs.push({ type: msg.type(), text: text.slice(0, 400), page: currentPage, time: Date.now() - startTime });
  });

  page.on("pageerror", (err) => {
    allLogs.push({ type: "PAGEERROR", text: `${err.name}: ${err.message.slice(0, 400)}`, page: currentPage, time: Date.now() - startTime });
  });
  ...
  console.log(`📄 דשבורד (idle 15s): ${idleAfter - idleBefore} console messages`);
  ...
  console.log("\n── By Page ──");
  for (const [pg, count] of Object.entries(byPage).sort((a, b) => b[1] - a[1])) {
  ...
  const uniqueWarnings = [...new Set(warnings.map(w => w.text.slice(0, 150)))];
  ...
  const uniqueInfoNormalized = [...new Set(infoLogs.map(l => {
    return l.text
      .replace(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, "<UUID>")
  ...
  console.log(`  Most repeated: [×${sorted[0]?.[1] || 0}] ${sorted[0]?.[0]?.slice(0, 80) || "none"}`);
  console.log("═".repeat(70));
});
