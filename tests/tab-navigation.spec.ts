...
test("Navigate between tabs after Gmail", async ({ page }) => {
  ...
  page.on("response", (res) => {
    if (res.status() >= 400) networkErrors.push(`${res.status()} ${res.url().substring(0, 120)}`);
  });
  ...
  const tabs = [
    ...
    { name: "Dashboard", path: "/", expectText: ["דשבורד", "לוח בקרה", "סיכום", "ברוך"] },
    { name: "Clients", path: "/clients", expectText: ["לקוחות", "חיפוש", "הוסף"] },
    { name: "Projects", path: "/projects", expectText: ["פרויקטים", "פרויקט"] },
    { name: "Tasks", path: "/tasks", expectText: ["משימות", "משימה"] },
    { name: "Calendar", path: "/calendar", expectText: ["לוח שנה", "יומן", "calendar"] },
    { name: "Reports", path: "/reports", expectText: ["דוחות", "דוח", "סטטיסטיקות"] },
    { name: "Settings", path: "/settings", expectText: ["הגדרות", "settings"] },
  ];

  const results: { name: string; status: string; url: string; errors: number; networkErrs: number; loadTime: number }[] = [];
  ...
  for (const tab of tabs) {
    ...
    if (currentUrl.includes("/auth")) {
      ...
      results.push({ name: tab.name, status: "SESSION_LOST", url: currentUrl, errors: 0, networkErrs: 0, loadTime });
    ...
    const hasExpectedContent = tab.expectText.some(text => 
      bodyText.includes(text) || bodyText.toLowerCase().includes(text.toLowerCase())
    );
    
    // Check for visible errors on page
    const hasErrorOnPage = bodyText.includes("שגיאה") || bodyText.includes("Error") || bodyText.includes("404");
    ...
    console.log(`${status} | ${tab.name} | URL: ${currentUrl} | Load: ${loadTime}ms | Console errors: ${newConsoleErrs} | Network errors: ${newNetworkErrs}`);
    ...
    // Take screenshot
    await page.screenshot({ path: `test-results/tab-${tab.name.toLowerCase()}.png`, fullPage: false });
  }

  // ===== RAPID SWITCHING TEST =====
  console.log("\n=== RAPID SWITCHING TEST ===");
  const rapidTabs = ["/", "/clients", "/gmail", "/projects", "/tasks", "/", "/calendar", "/clients", "/gmail"];
  ...
  console.log(`Rapid switch result: ${rapidSwitchOk ? "✅ OK" : "❌ Session lost"} | Final URL: ${finalUrl}`);
  ...
  for (const r of results) {
    console.log(`  ${r.status} | ${r.name.padEnd(12)} | ${r.loadTime}ms | Errs: ${r.errors} console, ${r.networkErrs} network`);
  }
