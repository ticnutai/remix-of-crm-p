...
test("Gmail - click connect button and observe", async ({ page }) => {
  ...
  if (await emailInput.isVisible({ timeout: 5000 })) {
    ...
    await page.screenshot({
      path: "test-results/gmail-login-not-found.png",
      fullPage: true,
    });
  }
  ...
  await page.screenshot({
    path: "test-results/gmail-before-connect.png",
    fullPage: true,
  });
  ...
  const hasAlreadyConnected =
    pageContent?.includes("דואר נכנס") ||
  ...
  console.log(
    "Gmail accounts in localStorage:",
    JSON.stringify(gmailAccountsStored, null, 2),
  );
  ...
  if (!connectButton) {
    ...
    await page.screenshot({
      path: "test-results/gmail-no-connect-button.png",
      fullPage: true,
    });
    ...
    console.log(
      "All buttons on page:",
      allButtons.filter((b) => b.trim()).join(" | "),
    );
    ...
    await page.screenshot({
      path: "test-results/gmail-after-connect.png",
      fullPage: true,
    });
  }
  ...
  const toasts = await page
    .locator(
      '[data-sonner-toast], [role="status"], [data-state="open"][role="alertdialog"], .toast, [data-radix-collection-item]',
    )
    .allTextContents();
  if (toasts.length > 0) {
  ...
  const errorBadges = await page
    .locator('[variant="destructive"], .text-destructive, .text-red-500')
    .allTextContents();
  ...
  const gmailLogs = consoleLogs.filter(
    (l) =>
