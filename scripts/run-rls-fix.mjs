...
async function runSqlViaRpc(token, sql, name) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/rpc/execute_safe_migration`,
    {
      method: "POST",
      headers: {
...
async function main() {
  ...
  for (const stmt of STATEMENTS) {
    ...
    if (result.ok) {
      ...
      const errMsg =
        result.data?.message || result.data?.raw || JSON.stringify(result.data);
  ...
  const testRes = await fetch(
    `${SUPABASE_URL}/rest/v1/chat_conversations?select=id,title&limit=5`,
    {
  ...
  if (Array.isArray(testData)) {
    ...
    testData.forEach((c) =>
      console.log(`   - ${c.id?.slice(0, 8)} ${c.title || "(no title)"}`),
    );
  } else {
