...
export function DataHubProvider({ children }: { children: React.ReactNode }) {
  ...
  const [normalizedData, setNormalizedData] = useState<NormalizedBackup | null>(null);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const [
        ...
        supabase.from('custom_tables').select('*', { count: 'exact', head: true }),
        supabase.from('custom_table_data').select('*', { count: 'exact', head: true }),
      ]);
      ...
      newStats.total = Object.values(newStats).reduce((a, b) => typeof b === 'number' ? a + b : a, 0);
      setStats(newStats);
    } catch (err) {
  ...
  // ============= Create Backup =============
  const createBackup = useCallback(async (format: 'json' | 'excel' | 'csv') => {
    setStatus('creating');
    setError(null);
    ...
    try {
      ...
      const backup = {
        metadata: {
          ...
          tables: Object.keys(data).reduce((acc, key) => ({ ...acc, [key]: data[key].length }), {}),
        },
        data,
      };
  ...
  const downloadBackup = useCallback(async (format: 'json' | 'excel' | 'csv') => {
  ...
  const restoreFromCloud = useCallback(async (backup: CloudBackup) => {
    ...
    try {
      const { data, error } = await supabase.storage.from('backups').download(backup.name);
      if (error) throw error;
  ...
  const deleteCloudBackup = useCallback(async (backup: CloudBackup) => {
    try {
      const { error } = await supabase.storage.from('backups').remove([backup.name]);
      if (error) throw error;
    ...
    try {
      ...
      if (parsedData.generated_at && parsedData.data) {
        ...
        if (normalized.data.clients) {
          setParsedClients(normalized.data.clients.map((c: any) => ({
        ...
        if (normalized.data.timeLogs) {
          setParsedTimeLogs(normalized.data.timeLogs.map((t: any) => ({
        ...
        for (const [key, value] of Object.entries(normalized.statistics)) {
          if (typeof value === 'number' && value > 0 && !['total', 'totalHours'].includes(key)) {
      ...
      // NCRM native format
      else if (parsedData.metadata?.source === 'ncrm-data-hub' || parsedData.data?.clients) {
        ...
        const normalized: NormalizedBackup = {
          metadata: {
            ...
            exportedAt: parsedData.metadata?.exportedAt || new Date().toISOString(),
          },
        ...
        if (normalized.data.timeLogs || parsedData.data?.time_entries) {
          setParsedTimeLogs(normalized.data.timeLogs || parsedData.data.time_entries);
        }
      }
      // Simple array of records
        ...
        if (sample) {
          ...
          if (fields.some(f => ['name', 'email', 'phone', 'client_name'].includes(f.toLowerCase()))) {
      ...
      result.canImport = result.entities.length > 0 || parsedClients.length > 0 || parsedTimeLogs.length > 0;
      result.estimatedTime = Math.ceil((result.entities.reduce((a, e) => a + e.count, 0) / 100) * 2);
      ...
      result.warnings.push(err instanceof Error ? err.message : 'שגיאה בניתוח הקובץ');
      setStatus('error');
    ...
    try {
      ...
      if (importOptions.time_entries && parsedTimeLogs.length > 0) {
        ...
        for (let i = 0; i < parsedTimeLogs.length; i++) {
          ...
          // Map client ID
          let clientId = log.client_id;
          if (clientId && entityMaps.clients[clientId]) {
            clientId = entityMaps.clients[clientId];
          }
