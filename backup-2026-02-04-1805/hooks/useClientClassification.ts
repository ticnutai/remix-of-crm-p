...
export function useClientClassification() {
  const [clientConsultants, setClientConsultants] = useState<Record<string, ClientConsultant[]>>({});
  ...
  const assignConsultant = useCallback(async (
    ...
    try {
      ...
      setClientConsultants(prev => ({
        ...prev,
        [clientId]: [
          ...(prev[clientId] || []).filter(c => c.consultant_id !== consultantId),
  ...
  const removeConsultant = useCallback(async (clientId: string, consultantId: string) => {
    try {
      setLoading(true);
      ...
      setClientConsultants(prev => ({
        ...prev,
        [clientId]: (prev[clientId] || []).filter(c => c.consultant_id !== consultantId)
      }));
  ...
  const updateClientClassification = useCallback(async (
    clientId: string,
  ...
  const getConsultantsForClient = useCallback((clientId: string): ClientConsultant[] => {
  ...
  const filterClients = useCallback(async (filter: ClientFilter) => {
    try {
      ...
      if (filter.consultantId) {
        ...
        const clientIds = new Set(consultantClients?.map(c => c.client_id) || []);
