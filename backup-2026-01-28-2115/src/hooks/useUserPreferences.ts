...
export const themePresets = {
  'navy-executive': { name: 'Navy Executive', primary: '220 45% 18%', secondary: '43 45% 60%', primaryHex: '#1B2541', secondaryHex: '#C9A962' },
  'ocean': { name: 'אוקיינוס', primary: '199 89% 48%', secondary: '213 47% 25%', primaryHex: '#0EA5E9', secondaryHex: '#1E3A5F' },
  'midnight': { name: 'חצות', primary: '222 47% 8%', secondary: '217 91% 60%', primaryHex: '#0A0F1C', secondaryHex: '#3B82F6' },
  'coral': { name: 'אלמוגים', primary: '25 95% 53%', secondary: '0 72% 51%', primaryHex: '#F97316', secondaryHex: '#DC2626' },
  'mint': { name: 'מנטה', primary: '160 84% 39%', secondary: '160 60% 52%', primaryHex: '#10B981', secondaryHex: '#34D399' },
  'warm': { name: 'חם', primary: '38 92% 50%', secondary: '21 90% 48%', primaryHex: '#F59E0B', secondaryHex: '#EA580C' },
  'sunset': { name: 'שקיעה', primary: '25 95% 53%', secondary: '0 84% 60%', primaryHex: '#F97316', secondaryHex: '#EF4444' },
  'forest': { name: 'יער', primary: '142 71% 45%', secondary: '142 64% 29%', primaryHex: '#22C55E', secondaryHex: '#15803D' },
  'lavender': { name: 'לבנדר', primary: '258 90% 76%', secondary: '258 90% 66%', primaryHex: '#A78BFA', secondaryHex: '#8B5CF6' },
  'luxury': { name: 'יוקרה', primary: '43 45% 60%', secondary: '220 45% 18%', primaryHex: '#C9A962', secondaryHex: '#1B2541' },
  'royal': { name: 'מלכותי', primary: '239 84% 67%', secondary: '239 84% 58%', primaryHex: '#6366F1', secondaryHex: '#4F46E5' },
  'cream': { name: 'קרם קלאסי', primary: '36 30% 76%', secondary: '30 20% 44%', primaryHex: '#D4C5B0', secondaryHex: '#8B7355' },
};
...
export function useUserPreferences() {
    ...
    try {
      ...
      if (data) {
        setPreferences({
          ...defaultPreferences,
          ...data,
          channels: typeof data.channels === 'object' ? data.channels : defaultPreferences.channels,
          notification_types: typeof data.notification_types === 'object' ? data.notification_types : defaultPreferences.notification_types,
        } as UserPreferences);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Apply font size
    root.style.setProperty('--font-size-multiplier', `${preferences.font_size / 100}`);
    
    // Apply line height
    const lineHeightMap = { compact: '1.4', normal: '1.6', spacious: '1.8' };
    root.style.setProperty('--line-height', lineHeightMap[preferences.line_height]);
    
    // Apply letter spacing
    const letterSpacingMap = { compact: '-0.02em', normal: '0', spacious: '0.02em' };
    root.style.setProperty('--letter-spacing', letterSpacingMap[preferences.letter_spacing]);

    // Apply theme colors
    const theme = themePresets[preferences.theme_preset as keyof typeof themePresets] || themePresets['navy-executive'];
    root.style.setProperty('--primary', preferences.custom_primary_color || theme.primary);
    root.style.setProperty('--secondary', preferences.custom_secondary_color || theme.secondary);

    // Apply advanced visual customization
    const borderRadiusMap = { none: '0', small: '0.25rem', medium: '0.5rem', large: '0.75rem', full: '9999px' };
    root.style.setProperty('--custom-radius', borderRadiusMap[preferences.border_radius]);
    
    const borderWidthMap = { none: '0', thin: '1px', normal: '2px', thick: '3px' };
    root.style.setProperty('--custom-border-width', borderWidthMap[preferences.border_width]);
    ...
    root.style.setProperty('--custom-shadow', shadowMap[preferences.shadow_intensity]);
    
    const animationSpeedMap = { none: '0ms', slow: '400ms', normal: '200ms', fast: '100ms' };
    root.style.setProperty('--custom-animation-speed', animationSpeedMap[preferences.animation_speed]);
