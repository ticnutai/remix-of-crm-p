...
const iconOptions = [
  { value: 'Phone', icon: Phone, label: 'טלפון' },
  { value: 'FolderOpen', icon: FolderOpen, label: 'תיקיה' },
  { value: 'Send', icon: Send, label: 'שליחה' },
...
interface SortableTaskProps {
  ...
  clientId: string;
  editingTask: { stageId: string; taskId: string; title: string } | null;
  setEditingTask: React.Dispatch<React.SetStateAction<{ stageId: string; taskId: string; title: string } | null>>;
    ...
    >
      ...
      <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button
          size="sm"
          variant="ghost"
          onClick={() => setEditingTask({ stageId, taskId: task.id, title: task.title })}
...
interface SortableStageItemProps {
  ...
  index: number;
  isEditing: boolean;
  editingStage: { stageId: string; name: string; icon: string } | null;
  setEditingStage: React.Dispatch<React.SetStateAction<{ stageId: string; name: string; icon: string } | null>>;
  ...
  return (
    ...
    >
      ...
      {isEditing ? (
            ...
            {iconOptions.map((opt) => {
              ...
              return (
                <Button
                  key={opt.value}
                  type="button"
                  variant={editingStage?.icon === opt.value ? "default" : "outline"}
                  size="sm"
                  onClick={() => setEditingStage(prev => prev ? { ...prev, icon: opt.value } : null)}
                  className="h-8 w-8 p-0"
                  title={opt.label}
                >
        ...
        <span className="flex-1 font-medium text-right">{stage.stage_name}</span>
      )}
      ...
      {!isEditing && (
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Button
            size="sm"
            variant="ghost"
            onClick={() => setEditingStage({ stageId: stage.stage_id, name: stage.stage_name, icon: stage.stage_icon || 'Phone' })}
            className="h-8 w-8 p-0"
            title="ערוך"
          >
            <Edit className="h-4 w-4" />
...
export function ClientStagesBoard({ clientId }: ClientStagesBoardProps) {
  ...
  const stageCompletionInfo = useMemo(() => {
    const info: Record<string, { isCompleted: boolean; progress: number }> = {};
    ...
    sortedStages.forEach(stage => {
      ...
      info[stage.stage_id] = { isCompleted, progress };
    });
  ...
  const expandedStageData = useMemo(() => 
    expandedStage ? sortedStages.find(s => s.stage_id === expandedStage) : null,
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-start gap-2 flex-wrap">
        ...
        <Button
          ...
          className="gap-2"
          style={showAllStages ? { backgroundColor: '#d4a843', color: '#1e293b' } : {}}
        >
          {showAllStages ? (
            <>
        ...
        {(showAllStages ? sortedStages : sortedStages.filter(s => s.stage_id === 'contact')).map((stage, index) => {
          ...
          return (
            <Card 
              key={stage.id}
              className={cn(
                "flex flex-col h-full transition-all duration-300 border-2 border-amber-400/60",
            ...
            >
              ...
              <div 
                className={cn(
                  ...
                  !isStageCompleted && !isActiveStage && "bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950"
                )}
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className={cn(
                    ...
                    isStageCompleted ? "bg-amber-100 dark:bg-amber-900/30" :
                    isActiveStage ? "bg-white/90" : "bg-white dark:bg-gray-800"
                  )}>
                    <Icon className={cn(
                      "h-5 w-5",
                      isStageCompleted ? "text-amber-600" :
                      isActiveStage ? "text-yellow-600" : "text-primary"
                    )} />
                  </div>
                </div>
                ...
                <h3 className={cn(
                  ...
                  isStageCompleted ? "text-amber-700 dark:text-amber-400" :
                  isActiveStage ? "text-white" : "text-foreground"
                )}>
                  {stage.stage_name}
                </h3>
                ...
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <div className="relative w-12 h-12">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle
                        ...
                        fill="none"
                        className={cn(
                          isStageCompleted ? "text-amber-200 dark:text-amber-900" :
                          isActiveStage ? "text-white/30" : "text-gray-200 dark:text-gray-700"
                        )}
                      />
                      <circle
                        cx="24"
                        ...
                        className={cn(
                          ...
                          isStageCompleted ? "text-amber-500" :
                  ...
                  <div className="text-right">
                    <Badge 
                      variant={isStageCompleted ? "secondary" : isActiveStage ? "secondary" : "outline"}
                      className={cn(
                        "text-xs",
                        isStageCompleted && "bg-amber-100 text-amber-700 border-amber-300",
                        isActiveStage && !isStageCompleted && "bg-white/90 text-yellow-700 border-0"
                      )}
                    >
              ...
              <CardContent className="flex-1 p-3 space-y-2 overflow-y-auto max-h-[500px]">
                {stage.tasks && stage.tasks.length > 0 ? (
                  <DndContext
                    ...
                    onDragEnd={(event) => handleDragEnd(event, stage.stage_id, stage.tasks || [])}
                  >
                    <SortableContext
                      ...
                      <div className="space-y-2">
                        {stage.tasks.map((task, index) => (
                          <SortableTaskItem
                            ...
                            stage={stage}
                            index={index}
                            showTaskCount={showTaskCount[stage.stage_id] || false}
                            clientId={clientId}
      ...
      <Dialog open={expandedStage !== null} onOpenChange={() => { setExpandedStage(null); clearSelection(); }}>
        <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] overflow-hidden flex flex-col" dir="rtl">
          {expandedStageData && (() => {
            ...
            const allSelected = expandedStageData.tasks && expandedStageData.tasks.length > 0 && 
              ...
              <>
                ...
                <div className={cn(
                  ...
                  <div className="flex items-center justify-between">
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => { setExpandedStage(null); clearSelection(); }}
                      className={cn(
                        "h-8 w-8 p-0",
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-left">
                        ...
                        <p className={cn(
                          "text-sm",
                          isFirstStage ? "text-white/80" : "text-muted-foreground"
                        )}>
                      ...
                      <div className={cn(
                        ...
                        isFirstStage ? "bg-white/90" : "bg-white dark:bg-gray-800"
                      )}>
                        <Icon className={cn(
                          "h-8 w-8",
                  ...
                  <div className="mt-4">
                    <div className={cn(
                      ...
                      isFirstStage ? "bg-white/30" : "bg-gray-200 dark:bg-gray-700"
                    )}>
                      <div 
                        className={cn(
                    ...
                    {expandedStageData.started_at && expandedStageData.target_working_days ? (
                      <>
                        <StageTimerDisplay
                          ...
                          onStyleChange={() => cycleStageTimerStyle(expandedStageData.stage_id)}
                          size="lg"
                          className={isFirstStage ? "bg-white/90 text-gray-800" : ""}
                        />
                        <Button
                          variant="ghost"
                          size="sm"
                          ...
                          onClick={() => stopStageTimer(expandedStageData.stage_id)}
                        >
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button
                            ...
                            className={cn(
                              "text-xs",
                              isFirstStage && "bg-white/90 hover:bg-white text-gray-800"
                            )}
                          >
                            ...
                            <div className="border-t pt-2 mt-2">
                              ...
                              <div className="flex gap-1">
                                <Input
                                  ...
                                  placeholder="ימים"
                                  className="h-7 text-xs text-center"
                                  value={customTimerDays?.stageId === expandedStageData.stage_id ? customTimerDays.days : ''}
                                  onChange={(e) => setCustomTimerDays({ stageId: expandedStageData.stage_id, days: e.target.value })}
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter' && customTimerDays?.days) {
                                      const days = parseInt(customTimerDays.days);
                                      if (days > 0 && days <= 365) {
                ...
                <Tabs value={expandedViewMode} onValueChange={(v) => setExpandedViewMode(v as 'cards' | 'table')} className="flex-1 flex flex-col overflow-hidden">
                  ...
                  <TabsContent value="cards" className="flex-1 overflow-y-auto space-y-3 mt-0">
                    {/* Select All */}
                    {expandedStageData.tasks && expandedStageData.tasks.length > 0 && (
                    ...
                    {expandedStageData.tasks && expandedStageData.tasks.length > 0 ? (
                      <DndContext
                        sensors={sensors}
                        collisionDetection={closestCenter}
                        onDragEnd={(event) => handleDragEnd(event, expandedStageData.stage_id, expandedStageData.tasks || [])}
                      >
                        <SortableContext
                            ...
                            <div key={task.id} className="flex items-center gap-2">
                              ...
                              <div className="flex-1">
                                <SortableExpandedTaskItem
                                  ...
                                  index={index}
                                  showTaskCount={showTaskCount[expandedStageData.stage_id] || false}
                                  clientId={clientId}
                                  setEditingTask={setEditingTask}
                                  handleToggleTask={handleToggleTask}
                                  handleDeleteTask={handleDeleteTask}
                            ...
                            {expandedStageData.tasks.map((task, index) => (
                              <tr 
                                ...
                                className={cn(
                                  ...
                                  task.completed ? "bg-green-50/50 dark:bg-green-950/20" : "hover:bg-muted/50",
                                ...
                                <td className="p-3">
                                  <div className="flex gap-1">
                                    ...
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      onClick={() => setEditingTask({ stageId: expandedStageData.stage_id, taskId: task.id, title: task.title })}
                ...
                <div className="pt-4 border-t flex gap-2">
                  <Button
                    ...
                    className="flex-1"
                    onClick={() => {
                      setAddingTask({ stageId: expandedStageData.stage_id, title: '' });
                    }}
                  >
                    <Plus className="h-4 w-4 ml-2" />
                    הוסף משימה
                  ...
                  <Button
                    variant="outline"
                    onClick={() => {
                      setBulkAddDialog({ stageId: expandedStageData.stage_id, tasks: '' });
                    }}
                  >
                  ...
                  <Button
                    variant="ghost"
                    onClick={() => toggleTaskCount(expandedStageData.stage_id)}
                    className={showTaskCount[expandedStageData.stage_id] ? "bg-primary/10" : ""}
                  >
                ...
                {iconOptions.map((opt) => {
                  ...
                  return (
                    <Button
                      key={opt.value}
                      type="button"
                      variant={newStageIcon === opt.value ? "default" : "outline"}
                      size="sm"
