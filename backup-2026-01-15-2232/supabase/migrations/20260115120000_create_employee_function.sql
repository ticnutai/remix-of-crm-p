...
BEGIN
  ...
  IF NOT public.is_admin(auth.uid()) THEN
    RETURN jsonb_build_object(
      'success', FALSE,
      'error', 'רק מנהלים יכולים להוסיף עובדים'
    );
  END IF;
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    RETURN jsonb_build_object(
      'success', TRUE,
      'message', 'משתמש קיים ' || p_email || ' נוסף כעובד בהצלחה',
      'user_id', v_user_id,
  ...
  ELSE
    ...
    RETURN jsonb_build_object(
      'success', FALSE,
      'error', 'משתמש לא קיים. נא ליצור תחילה ב-Supabase Dashboard או דרך Auth UI',
      'hint', 'לך ל-Authentication -> Users -> Invite user'
    );
  END IF;
