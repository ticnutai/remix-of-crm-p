...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, svix-id, svix-timestamp, svix-signature",
};
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `Updated email log ${emailLog.id} with status: ${updates.status}`,
    );
