...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `Found ${pendingEmails?.length || 0} pending emails to process`,
    );
      ...
      for (const email of pendingEmails) {
        ...
        if (sendResult.success) {
          ...
          if (newRetryCount < email.max_retries) {
            ...
            results.retried++;
            console.log(
              `⚠️ Email ${email.id} failed, scheduled for retry #${newRetryCount} at ${nextRetryTime}`,
            );
          } else {
            ...
            results.failed++;
            console.log(
              `❌ Email ${email.id} failed permanently after ${newRetryCount} attempts`,
            );
          }
        }
