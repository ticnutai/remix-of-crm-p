...
serve(async (req) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      throw new Error(
        `Failed to authenticate with Green Invoice: ${errorText}`,
      );
    }
    ...
    switch (action) {
      case "create_invoice": {
        ...
        console.log(
          "Creating invoice in Green Invoice:",
        ...
        const invoicePayload = {
          description:
            invoice.description || `חשבונית ${invoice.invoice_number}`,
      ...
      case "list_invoices": {
        ...
        console.log(
          `Found ${result.items?.length || 0} documents from Green Invoice`,
        );
        break;
      }
      ...
      case "list_all_invoices": {
        ...
        do {
          ...
          console.log(
            `Page ${currentPage}/${totalPages}: Got ${pageResult.items?.length || 0} items. Total so far: ${allItems.length}`,
          );
      ...
      case "get_document_link": {
        ...
        const downloadResponse = await fetch(
          `${baseUrl}/documents/${documentId}/download/links`,
          {
            method: "GET",
            headers: authHeaders,
          },
      ...
      case "download_and_store_pdf": {
        ...
        const downloadLinksResponse = await fetch(
          `${baseUrl}/documents/${documentId}/download/links`,
          {
            method: "GET",
            headers: authHeaders,
      ...
      case "sync_all_invoices_pdfs": {
        ...
        for (const inv of invoicesToSync || []) {
          try {
            console.log(
              `Processing invoice ${inv.invoice_number} (${inv.green_invoice_id})...`,
            );

            // Get download link
            const downloadLinksResponse = await fetch(
              `${baseUrl}/documents/${inv.green_invoice_id}/download/links`,
              {
                method: "GET",
                headers: authHeaders,
              },
            ...
            if (!downloadLinksResponse.ok) {
              throw new Error(
                `Failed to get download link: ${downloadLinksResponse.status}`,
              );
            }
            ...
            console.error(
              `✗ Failed to sync invoice ${inv.invoice_number}:`,
              err.message,
            );
          }
        }
        ...
        console.log(
          `Sync complete: ${results.success}/${results.total} succeeded, ${results.failed} failed`,
        );
        result = results;
        break;
      }
