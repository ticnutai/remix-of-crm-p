...
serve(async (req) => {
  ...
  try {
    ...
    const { Client } =
      await import("https://deno.land/x/postgres@v0.17.0/mod.ts");
    ...
    try {
      ...
      const tableMatches =
        sql.match(/(?:FROM|INTO|UPDATE|TABLE|ON)\s+(?:public\.)?(\w+)/gi) || [];
      affectedTables = [
        ...new Set(
          tableMatches.map((m) => {
      ...
      if (mode === "explain") {
        ...
        try {
          const explainResult = await client.queryObject(
            `EXPLAIN (ANALYZE false, FORMAT JSON) ${sql.split(";")[0]}`,
          );
          ...
          return new Response(
            JSON.stringify({
              success: false,
              error:
                explainError.message || "Could not generate execution plan",
              mode: "explain",
              execution_time_ms: Date.now() - startTime,
            } as ExecuteResponse),
            {
...
function parsePostgresError(error: any): {
  message: string;
  code?: string;
  position?: number;
  hint?: string;
