...
-- ============================================================================
-- PART 0: Add column to quotes first (before creating contracts table)
-- ============================================================================
...
-- Update link in quotes table (add FK constraint if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ============================================================================
-- PART 2: Payment Schedules Table
-- ============================================================================

CREATE TABLE IF NOT EXISTS payment_schedules (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  contract_id uuid NOT NULL REFERENCES contracts(id) ON DELETE CASCADE,
  invoice_id uuid REFERENCES invoices(id), -- Link when invoice is created
...
-- ============================================================================
-- PART 3: Contract Documents Table
-- ============================================================================
...
-- ============================================================================
-- PART 4: Contract Amendments Table
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS contract_amendments (
  ...
  previous_value text,
  new_value text,
  value_change_amount numeric, -- Change in contract value (can be negative)
...
-- Indexes for contract_amendments
...
-- Trigger: Update updated_at for contracts
...
-- Trigger: Update updated_at for payment_schedules
...
-- Trigger: Log payment schedule activity
...
COMMENT ON COLUMN contracts.contract_type IS 'Contract type: fixed_price, hourly, milestone, retainer';
COMMENT ON COLUMN contracts.status IS 'Status: draft, active, completed, terminated, expired';
COMMENT ON COLUMN payment_schedules.status IS 'Status: pending, sent, paid, overdue, cancelled, partial';
