-- ייבוא 4 לקוחות חסרים ו-5 לוגים שלהם
-- נוצר אוטומטית ב-2026-01-27
...
BEGIN
    ...
    -- שלב 1: יצירת הלקוחות החסרים
    INSERT INTO clients (name, name_clean, original_id, status, source, user_id, created_by, created_at)
    SELECT 'בלינצקי', 'בלינצקי', '68b1abad175af64002301909', 'active', 'import', v_user_id, v_user_id, NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients WHERE original_id = '68b1abad175af64002301909');
    
    INSERT INTO clients (name, name_clean, original_id, status, source, user_id, created_by, created_at)
    SELECT 'בלניצקי רחל', 'בלניצקי רחל', '68dbdea396fc743b797f63f0', 'active', 'import', v_user_id, v_user_id, NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients WHERE original_id = '68dbdea396fc743b797f63f0');
    
    INSERT INTO clients (name, name_clean, original_id, status, source, user_id, created_by, created_at)
    SELECT 'ווגנר', 'ווגנר', '68bee95f66229a0168b67de1', 'active', 'import', v_user_id, v_user_id, NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients WHERE original_id = '68bee95f66229a0168b67de1');
    
    INSERT INTO clients (name, name_clean, original_id, status, source, user_id, created_by, created_at)
    SELECT 'הכהן יוסף יצחק', 'הכהן יוסף יצחק', '68bee95f66229a0168b67dde', 'active', 'import', v_user_id, v_user_id, NOW()
    WHERE NOT EXISTS (SELECT 1 FROM clients WHERE original_id = '68bee95f66229a0168b67dde');
END $$;
...
BEGIN
    ...
    -- לוג 1: בלינצקי - 2025-10-30 - 1.3 דקות (80 שניות)
    ...
    -- לוג 2: בלינצקי - 2025-10-30 - 0.2 דקות (10 שניות)
    SELECT id INTO v_client_id FROM clients WHERE original_id = '68b1abad175af64002301909' LIMIT 1;
    IF v_client_id IS NOT NULL THEN
        INSERT INTO time_entries (user_id, client_id, description, start_time, end_time, is_billable, is_running, tags, custom_data, created_at)
        VALUES (v_user_id, v_client_id, 'רישום זמן מיובא', 
    ...
    -- לוג 3: בלניצקי רחל - 2025-09-30 - 11.9 דקות (714 שניות)
    ...
    -- לוג 4: ווגנר - 2025-09-09 - 48.9 דקות (2934 שניות)
    ...
    -- לוג 5: הכהן יוסף יצחק - 2025-09-08 - 72.6 דקות (4356 שניות)
...
-- סיכום
SELECT 'לקוחות חדשים:', COUNT(*) FROM clients WHERE original_id IN ('68b1abad175af64002301909', '68dbdea396fc743b797f63f0', '68bee95f66229a0168b67de1', '68bee95f66229a0168b67dde');
SELECT 'סה"כ לוגים מיובאים:', COUNT(*) FROM time_entries WHERE (custom_data->>'imported_from') = 'backup';
