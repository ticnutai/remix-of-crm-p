...
export function useGoogleContacts() {
  ...
  // Fetch contacts
  const fetchContacts = useCallback(async (pageSize: number = 100) => {
    if (!user) return [];
  ...
  const importContactAsClient = useCallback(async (contact: GoogleContact): Promise<string | null> => {
    if (!user) return null;
  ...
  const importMultipleContacts = useCallback(async (
    contactsToImport: GoogleContact[]
  ): Promise<number> => {
  ...
  // Search contacts
  const searchContacts = useCallback((query: string): GoogleContact[] => {
