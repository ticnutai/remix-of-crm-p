...
export function useClientStagesTable() {
  const [clientStages, setClientStages] = useState<Map<string, ClientStageInfo>>(new Map());
  ...
  const fetchClientStages = useCallback(async () => {
    try {
      ...
      const tasksByClient = new Map<string, { completed: number; total: number }>();
      (tasks || []).forEach(task => {
        const stats = tasksByClient.get(task.client_id) || { completed: 0, total: 0 };
        stats.total++;
      ...
      stagesByClient.forEach((clientStages, clientId) => {
        ...
        const tasksByStage = new Map<string, { completed: number; total: number }>();
        clientTasks.forEach(t => {
          const stats = tasksByStage.get(t.stage_id) || { completed: 0, total: 0 };
          stats.total++;
        ...
        const taskStats = tasksByClient.get(clientId) || { completed: 0, total: 0 };
  ...
  const updateClientStage = useCallback(async (clientId: string, newStageName: string) => {
    try {
      // Get client's stages
      ...
      if (!stages || stages.length === 0) {
        ...
        // Check if stage with this name exists
        const existingStage = stages.find(s => s.stage_name === newStageName);
        ...
        // If stage exists, mark previous stages as complete
        // and move to the selected stage
        
        // This requires marking tasks as complete for stages before the target
        if (existingStage) {
          const targetOrder = existingStage.sort_order;
          const previousStages = stages.filter(s => s.sort_order < targetOrder);
          
          // Mark all tasks in previous stages as complete
  ...
  useEffect(() => {
    const stagesChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'client_stages' }, () => {
        fetchClientStages();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'client_stage_tasks' }, () => {
        fetchClientStages();
      })
      .subscribe();
