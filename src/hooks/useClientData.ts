...
export function useClientData(clientId: string | undefined) {
  ...
  const fetchReminders = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("reminders")
      .select("*")
      .or(`client_id.eq.${clientId},entity_id.eq.${clientId}`)
  ...
  const fetchAllClientTables = useCallback(async () => {
    ...
    if (customRows.length > 0) {
      ...
      const tableGroups = new Map<
        string,
        { displayName: string; rows: Record<string, any>[] }
      >();
      ...
      customRows.forEach((row) => {
        ...
        tableGroups
          .get(row.table_id)!
          .rows.push({ ...row.data, id: row.id, created_at: row.created_at });
      });
      ...
      for (const [tableId, group] of tableGroups) {
        ...
        const columns = ((tableData?.columns as any[]) || []).map(
          (col: any) => ({
  ...
  const updateClient = useCallback(
    async (updates: Partial<ClientDetails>) => {
      ...
      logToActivityLog(user?.id, "update", "clients", clientId, {
        changes: updates,
      });
      await fetchClient();
  ...
  const addInvoice = useCallback(
    async (invoice: {
      ...
      if (error) {
        toast({
          title: "שגיאה",
          description: "לא ניתן ליצור חשבונית",
          variant: "destructive",
        });
        return false;
      }
  ...
  const updateInvoiceStatus = useCallback(
    async (invoiceId: string, status: string, paidDate?: string) => {
      ...
      if (error) {
        toast({
          title: "שגיאה",
          description: "לא ניתן לעדכן את החשבונית",
          variant: "destructive",
        });
        return false;
      }
