...
export function useGoogleDrive() {
  ...
  const listFiles = useCallback(
    async (folderId: string = "root", pageSize: number = 20) => {
      if (!user) return [];
      ...
      try {
        ...
        const formattedFiles: DriveFile[] = (data.files || []).map(
          (file: any) => ({
            id: file.id,
            name: file.name,
        ...
        const filesList = formattedFiles.filter(
  ...
  const uploadFile = useCallback(
    async (file: File, folderId?: string): Promise<DriveFile | null> => {
      if (!user) return null;
  ...
  const createFolder = useCallback(
    async (name: string, parentId?: string): Promise<DriveFolder | null> => {
      if (!user) return null;
  ...
  // Link file to client
  const linkFileToClient = useCallback(
    async (file: DriveFile, clientId: string) => {
