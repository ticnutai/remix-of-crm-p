...
export function useEmailFolders() {
  ...
  const updateFolder = useCallback(
    async (
      folderId: string,
      updates: Partial<
        Pick<EmailFolder, "name" | "color" | "icon" | "sort_order">
      >,
    ) => {
  ...
  // Add email to folder
  const addEmailToFolder = useCallback(
    async (
      folderId: string,
      email: GmailMessage,
      ...
      try {
        const { error } = await supabase.from("email_folder_items").upsert(
          {
            user_id: user.id,
            folder_id: folderId,
            email_id: email.id,
  ...
  const batchAddEmailsToFolder = useCallback(
    async (folderId: string, emails: GmailMessage[]) => {
  ...
  const removeEmailFromFolder = useCallback(
    async (folderId: string, emailId: string) => {
      if (!user?.id) return false;
  ...
  const getEmailsInFolder = useCallback(
    async (folderId: string) => {
      if (!user?.id) return [];
  ...
  const autoClassifyEmail = useCallback(
    async (email: GmailMessage) => {
      ...
      for (const rule of sortedRules) {
        ...
        switch (rule.rule_type) {
          ...
          case "sender_name":
            matches =
              senderEmail.includes(rule.rule_value) ||
  ...
  const batchAutoClassify = useCallback(
    async (emails: GmailMessage[]) => {
  ...
  const getClientFolder = useCallback(
    (clientId: string) => {
  ...
  const createClientAutoRule = useCallback(
