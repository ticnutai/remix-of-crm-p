...
export function useConsultants() {
  ...
  const addConsultant = async (consultant: Omit<Consultant, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      const { data, error } = await supabase
        .from('consultants')
        .insert(consultant)
      ...
      setConsultants(prev => [...prev, data as Consultant].sort((a, b) => a.name.localeCompare(b.name)));
      toast({
        title: 'הצלחה',
  ...
  const updateConsultant = async (id: string, updates: Partial<Consultant>) => {
    try {
      ...
      setConsultants(prev => prev.map(c => c.id === id ? (data as Consultant) : c));
      toast({
        title: 'הצלחה',
...
export function useTaskConsultants(taskId: string) {
  ...
  const linkConsultant = async (consultantId: string, keyword: string, keywordContext?: string) => {
    try {
      const { data, error } = await supabase
        .from('task_consultants')
        .insert({
  ...
  const unlinkConsultant = async (taskConsultantId: string) => {
    try {
      ...
      setTaskConsultants(prev => prev.filter(tc => tc.id !== taskConsultantId));
      toast({
        title: 'הצלחה',
  ...
  const getConsultantForKeyword = (keyword: string): TaskConsultant | undefined => {
