...
export function useEmployeesSync() {
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      ...
      const [profilesResult, rolesResult, timeEntriesResult] =
        await Promise.all([
          supabase
            .from("profiles")
            .select("*")
          ...
          supabase
            .from("time_entries")
            .select(
              "user_id, start_time, end_time, duration_minutes, is_billable, is_running",
            )
        ...
        .map((profile) => {
          ...
          return {
            ...
            billableHoursThisMonth:
              Math.round(stats.billableHoursThisMonth * 10) / 10,
      ...
      try {
        localStorage.setItem(
          "cachedEmployees",
          JSON.stringify(syncedEmployees),
        );
      } catch (e) {
  ...
  }, [user?.id, toast, calculateTimeStats]);
