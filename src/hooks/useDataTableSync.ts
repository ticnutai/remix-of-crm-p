...
function withTimeout<T>(
  promiseOrBuilder: any,
  timeoutMs: number,
  operation: string,
): Promise<T> {
  const thenable = Promise.resolve(promiseOrBuilder as PromiseLike<T>);
  return Promise.race([
    thenable,
...
export function useDataTableSync() {
  ...
  const [projects, setProjects] =
    useState<SyncedProject[]>(getInitialProjects());
    ...
    try {
      ...
      const [projectsRes, clientsRes] = await Promise.all([
        supabase
          .from("projects")
          .select(
            "id, name, client_id, status, priority, budget, start_date, end_date, description, assigned_to",
          )
      ...
      if (projectsRes.data) {
        ...
        const transformedProjects: SyncedProject[] = projectsRes.data.map(
          (p: any) => ({
            id: p.id,
            name: p.name,
            client:
              clientsRes.data?.find((c: any) => c.id === p.client_id)?.name ||
              "ללא לקוח",
  ...
  const updateProject = useCallback(
    async (projectId: string, columnId: string, newValue: any) => {
      if (!isManager) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה לעדכן פרויקטים",
          variant: "destructive",
        });
        return false;
      }
  ...
  const addProject = useCallback(
    async (name: string, clientName: string) => {
      if (!isManager || !user) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה להוסיף פרויקטים",
          variant: "destructive",
        });
        return null;
      }
  ...
  const deleteProject = useCallback(
    async (projectId: string) => {
      if (!isManager) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה למחוק פרויקטים",
          variant: "destructive",
        });
        return false;
      }
  ...
  const updateClient = useCallback(
    async (clientId: string, columnId: string, newValue: any) => {
      if (!isManager) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה לעדכן לקוחות",
          variant: "destructive",
        });
        return false;
      }
  ...
  const addClient = useCallback(
    async (name: string) => {
      if (!isManager || !user) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה להוסיף לקוחות",
          variant: "destructive",
        });
        return null;
      }
      ...
      try {
        ...
        setClientsMap((prev) => ({
          ...prev,
          [syncedClient.id]: syncedClient.name,
        }));
  ...
  const deleteClient = useCallback(
    async (clientId: string) => {
      if (!isManager) {
        toast({
          title: "שגיאה",
          description: "אין לך הרשאה למחוק לקוחות",
          variant: "destructive",
        });
        return false;
      }
        ...
        setProjects((prev) =>
          prev.map((p) =>
            p.client_id === clientId
              ? { ...p, client: "ללא לקוח", client_id: null }
              : p,
          ),
        );
