...
import { analyzeSql, parseError, getRiskColor, getRiskBadgeVariant, getRiskLabel, tokenizeSql } from '@/utils/sqlAnalyzer';

describe('SQL Analyzer', () => {
  // ==================== analyzeSql ====================
  describe('analyzeSql', () => {
    it('should detect CREATE TABLE operation', () => {
      const result = analyzeSql('CREATE TABLE users (id UUID PRIMARY KEY, name TEXT);');
    ...
    it('should warn about DELETE without WHERE', () => {
      ...
      expect(result.warnings.some(w => w.includes('DELETE') && w.includes('WHERE'))).toBe(true);
    });

    it('should NOT warn about DELETE with WHERE', () => {
      const result = analyzeSql("DELETE FROM clients WHERE id = '123';");
      expect(result.warnings.some(w => w.includes('DELETE') && w.includes('WHERE'))).toBe(false);
    });

    it('should warn about UPDATE without WHERE', () => {
      const result = analyzeSql("UPDATE clients SET status = 'active';");
      expect(result.warnings.some(w => w.includes('UPDATE') && w.includes('WHERE'))).toBe(true);
    });
    ...
    it('should suggest IF NOT EXISTS for CREATE TABLE', () => {
      ...
      expect(result.warnings.some(w => w.includes('IF NOT EXISTS'))).toBe(true);
    });

    it('should NOT suggest IF NOT EXISTS when already present', () => {
      const result = analyzeSql('CREATE TABLE IF NOT EXISTS new_table (id UUID);');
      expect(result.warnings.some(w => w.includes('IF NOT EXISTS'))).toBe(false);
    });
