...
const SYNCABLE_TABLES: Set<string> = new Set([
  'clients', 'projects', 'tasks', 'meetings', 'time_entries',
  'documents', 'quotes', 'contracts', 'invoices', 'reminders',
  'contacts', 'calls', 'notes',
]);
    ...
    if (navigator.onLine) {
      try {
        ...
        if (operation === 'delete' && resultId) {
          offlineStorage.delete(table as StoreName, resultId).catch(console.warn);
        } else if (result && typeof result === 'object' && 'id' in (result as any)) {
          offlineStorage.set(table as StoreName, result as any).catch(console.warn);
        }
        return result;
      } catch (error) {
        // If online but request failed, queue for later
        console.warn(`⚠️ Mutation failed for ${table}, queueing for sync:`, error);
