...
describe("DataValidation", () => {
  ...
  describe("isValidEmail", () => {
    it("should accept valid emails", () => {
      ...
      expect(DataValidation.isValidEmail("user.name@domain.co.il").valid).toBe(
        true,
      );
    ...
    it("should reject invalid format", () => {
      ...
      expect(DataValidation.isValidEmail("spaces in@email.com").valid).toBe(
        false,
      );
    });

    it("should reject emails with dangerous characters", () => {
      expect(DataValidation.isValidEmail("<script>@evil.com").valid).toBe(
        false,
      );
      expect(
        DataValidation.isValidEmail("test@example.com onclick=alert(1)").valid,
      ).toBe(false);
    });
  });
  ...
  describe("containsDangerousCharacters", () => {
    it("should detect script tags", () => {
      expect(
        DataValidation.containsDangerousCharacters("<script>alert(1)</script>"),
      ).toBe(true);
    });

    it("should detect javascript URLs", () => {
      expect(
        DataValidation.containsDangerousCharacters("javascript:alert(1)"),
      ).toBe(true);
    });

    it("should detect event handlers", () => {
      expect(
        DataValidation.containsDangerousCharacters("onclick=doEvil()"),
      ).toBe(true);
      expect(DataValidation.containsDangerousCharacters("onerror=hack()")).toBe(
        true,
      );
    });

    it("should detect SQL injection patterns", () => {
      expect(DataValidation.containsDangerousCharacters("' OR '1'='1")).toBe(
        true,
      );
      expect(
        DataValidation.containsDangerousCharacters("; DROP TABLE users"),
      ).toBe(true);
      expect(
        DataValidation.containsDangerousCharacters(
    ...
    it("should detect iframe injection", () => {
      expect(
        DataValidation.containsDangerousCharacters('<iframe src="evil.com">'),
      ).toBe(true);
    });

    it("should allow safe text", () => {
      expect(DataValidation.containsDangerousCharacters("שלום עולם")).toBe(
        false,
      );
      expect(DataValidation.containsDangerousCharacters("John Doe")).toBe(
        false,
      );
      expect(
        DataValidation.containsDangerousCharacters("רחוב הרצל 5, תל אביב"),
      ).toBe(false);
    });
  });
  ...
  describe("sanitizeInput", () => {
    ...
    it("should remove javascript URLs", () => {
      expect(DataValidation.sanitizeInput("javascript:alert(1)")).toBe(
        "alert(1)",
      );
    });
  ...
  describe("isValidURL", () => {
    it("should accept valid URLs", () => {
      ...
      expect(DataValidation.isValidURL("http://www.site.co.il").valid).toBe(
        true,
      );
    });
  ...
  describe("validateObject", () => {
    it("should validate with custom rules", () => {
      const result = DataValidation.validateObject(
        ...
        {
          name: (v) =>
            v ? { valid: true } : { valid: false, error: "Name required" },
