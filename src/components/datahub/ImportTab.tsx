...
export function ImportTab({ onComplete }: ImportTabProps) {
  ...
  const [step, setStep] = useState<
    "upload" | "analyze" | "map" | "preview" | "import" | "complete"
  >("upload");
  ...
  const parseExcelWorkbook = async (
  ...
  const mapSheetNameToEntity = (sheetName: string): string => {
    ...
    if (nameLower.includes("client") || nameLower.includes("לקוח"))
      return "clients";
    if (nameLower.includes("project") || nameLower.includes("פרויקט"))
      return "projects";
    if (nameLower.includes("time") || nameLower.includes("זמן"))
      return "time_entries";
    if (nameLower.includes("task") || nameLower.includes("משימ"))
      return "tasks";
    if (nameLower.includes("meeting") || nameLower.includes("פגיש"))
      return "meetings";
    if (nameLower.includes("quote") || nameLower.includes("הצע"))
      return "quotes";
    return sheetName;
  };
  ...
  const guessEntityTypeFromFields = (fields: string[]): string => {
    ...
    if (fieldsLower.some((f) => f.includes("client") || f.includes("לקוח")))
      return "clients";
    if (fieldsLower.some((f) => f.includes("duration") || f.includes("משך")))
      return "time_entries";
    if (fieldsLower.some((f) => f.includes("task") || f.includes("משימ")))
      return "tasks";
  ...
  const importSingleRecord = async (
    ...
    if (
      mapping.strategy !== "always_insert" &&
      mapping.matchFields.length > 0
    ) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            נמצאו {analysis?.totalRecords || 0} רשומות ב-
            {analysis?.entities.length || 0} קטגוריות
          </CardDescription>
        </CardHeader>
      </Card>
    ...
    <div className="space-y-6">
          ...
          {importProgress?.errors && importProgress.errors.length > 0 && (
            <ScrollArea className="h-40 border rounded-lg p-2">
              {importProgress.errors.map((err, idx) => (
                <div
                  key={idx}
                  className="text-sm text-red-600 py-1 border-b last:border-0"
                >
                  <span className="font-medium">
                    {ENTITY_NAMES_HE[err.entity]}:
                  </span>{" "}
                  {err.message}
                </div>
              ))}
      ...
      {importProgress?.phase === "complete" && (
        <div className="flex justify-center gap-4">
          ...
          <Button onClick={onComplete}>סיום</Button>
        </div>
      )}
    </div>
  );
