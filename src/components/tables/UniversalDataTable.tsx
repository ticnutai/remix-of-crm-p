...
export interface UniversalDataTableProps<
  T extends { id: string; custom_data?: Record<string, any> },
> {
  // Basic props
...
>({
  ...
  const handleHideColumn = useCallback((columnId: string) => {
    ...
    toast({
      title: "עמודה הוסתרה",
      description: "ניתן להציג אותה שוב מתפריט העמודות",
    });
  }, []);
    ...
    return customColumns.map((col) => {
      ...
      switch (col.column_type) {
        ...
        case "select":
          ...
          baseColumn.editOptions = (col.column_options || []).map(
            (opt: string) => ({
              value: opt,
              label: opt,
              color: "#6b7280",
        ...
        case "data_type":
          ...
          baseColumn.editType = "enhanced-select";
          baseColumn.editOptions = (dataTypeOptions[col.column_key] || []).map(
            (opt) => ({
              value: opt.value,
              label: opt.label,
              color: opt.color,
  ...
  const emptyCustomColumns = useMemo(() => {
    return customColumns.filter(
      (col) => !columnsWithData.has(col.column_key) && col.is_required !== true,
    );
  ...
  const handleCellEdit = useCallback(
    async (row: T, columnId: string, value: any) => {
  ...
  const handleBulkColumnsAdded = useCallback(
    async (columns: CustomColumn[], groupName?: string) => {
  ...
  const handleTemplateColumnsAdded = useCallback(
    (columns: any[]) => {
  ...
  const generateColumnKey = useCallback(
    (name: string, existingKeys: string[]): string => {
  ...
  const handleQuickAddColumns = useCallback(
    async (columns: Array<{ name: string; type: string }>) => {
      try {
  ...
  return (
    <div className="space-y-0">
      ...
      <DataTable
        ...
        onQuickAddRows={
          canAddColumns ? () => setIsQuickAddRowsOpen(true) : undefined
        }
        onQuickAddColumns={
          canAddColumns ? () => setIsQuickAddColumnsOpen(true) : undefined
        }
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          setDeleteColumnDialog({
            open,
            column: open ? deleteColumnDialog.column : null,
          })
        }
      >
