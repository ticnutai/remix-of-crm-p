...
export function BankTransactionsImport({
  onImportComplete,
}: BankTransactionsImportProps) {
  const { user } = useAuth();
  ...
  const autoDetectColumns = useCallback((headerRow: string[]) => {
    ...
    headerRow.forEach((header) => {
      ...
      (Object.keys(COLUMN_PATTERNS) as Array<keyof ColumnMapping>).forEach(
        (key) => {
          if (!newMapping[key]) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const headerRow = (jsonData[0] as unknown as string[]).map((h) =>
        String(h || "").trim(),
      );
      ...
      const transactions = rows
        .map((row) => {
          ...
          return {
            ...
            value_date: columnMapping.value_date
              ? parseDate(row[columnMapping.value_date])
              : null,
            description: String(row[columnMapping.description] || ""),
            reference_number: columnMapping.reference_number
              ? String(row[columnMapping.reference_number] || "")
              : null,
            debit: columnMapping.debit
              ? parseNumber(row[columnMapping.debit])
              : 0,
            credit: columnMapping.credit
              ? parseNumber(row[columnMapping.credit])
              : 0,
            balance: columnMapping.balance
              ? parseNumber(row[columnMapping.balance])
              : null,
      ...
      >
        ...
        {step === "mapping" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {(
                Object.keys(HEBREW_COLUMN_NAMES) as Array<keyof ColumnMapping>
              ).map((field) => (
                <div key={field}>
            ...
            <div className="flex justify-between">
              ...
              <Button
                ...
                disabled={
                  !columnMapping.transaction_date || !columnMapping.description
                }
              >
                תצוגה מקדימה
              </Button>
              ...
              <Table>
                ...
                <TableBody>
                  {previewRows.map((row, index) => (
                    <TableRow key={index}>
                      <TableCell>
                        {parseDate(row[columnMapping.transaction_date]) || "-"}
                      </TableCell>
