          ...
          <div className="text-center space-y-4">
            ...
            <div className="flex items-center justify-center gap-4 text-sm text-muted-foreground">
              {content.documentNumber && (
                <span>מסמך: {content.documentNumber}</span>
              )}
              {content.date && (
                <span>
                  תאריך: {new Date(content.date).toLocaleDateString("he-IL")}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {content.parties?.map((party: any, index: number) => (
              ...
              >
                ...
                {party.idNumber && (
                  <p className="text-sm">ת.ז/ח.פ: {party.idNumber}</p>
                )}
                {party.address && (
                  <p className="text-sm">כתובת: {party.address}</p>
                )}
            ...
            <table className="w-full border-collapse">
                ...
                {content.items?.map((item: any, index: number) => (
                  <tr key={index} className="border-b">
                    ...
                    <td className="text-left p-2">
                      {item.price?.toLocaleString("he-IL")} {currencySymbol}
                    </td>
                    <td className="text-left p-2 font-semibold">
                      {item.total?.toLocaleString("he-IL")} {currencySymbol}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="font-semibold">
                  ...
                  <td className="text-left p-2">
                    {subtotal.toLocaleString("he-IL")} {currencySymbol}
                  </td>
                </tr>
                {content.showVat && (
                  <tr className="font-semibold">
                    <td colSpan={4} className="text-left p-2">
                      מע"מ ({content.vatRate}%):
                    </td>
                    <td className="text-left p-2">
                      {vat.toLocaleString("he-IL")} {currencySymbol}
                    </td>
                  </tr>
                )}
                ...
                <div key={index} className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      תאריך:{" "}
                      {signature.date
                        ? new Date(signature.date).toLocaleDateString("he-IL")
                        : "_____"}
                    </p>
                  </div>
                </div>
