...
export function EdgeFunctionsManager() {
  ...
  const [selectedFunction, setSelectedFunction] =
    useState<FunctionStatus | null>(null);
  ...
  const [expandedCategory, setExpandedCategory] =
    useState<FunctionCategory | null>(null);
  ...
  const testFunction = useCallback(async (fn: FunctionStatus) => {
    ...
    try {
      ...
      setFunctions((prev) =>
        prev.map((f) => (f.name === fn.name ? { ...f, status: "error" } : f)),
      );
  ...
  const downloadReport = useCallback(() => {
    const now = new Date();
    const successResults = testResults.filter(
        ...
        possibleCauses:
          r.status === 0
            ? [
                ...
                "שגיאת רשת / CORS",
                "URL שגוי",
                "הפונקציה לא קיימת ב-Supabase",
              ]
            : r.status >= 500
              ? [
                  "שגיאה פנימית בפונקציה",
                  "חסרים משתני סביבה",
                  "באג בקוד הפונקציה",
                  "תלות חסרה (import)",
                ]
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl flex items-center gap-2">
                  ⚡ Edge Functions
                  <Badge
                    className={cn(goldGradient, "text-white border-0 text-xs")}
                  >
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            <div className="text-sm space-y-1">
              ...
              <p className="text-blue-600/80 dark:text-blue-400/80">
                Edge Functions נפרסות <strong>אוטומטית</strong> כשקוד נשמר דרך
                Lovable. כל שינוי בתיקיית{" "}
                <code className="bg-blue-500/20 px-1.5 py-0.5 rounded text-xs">
                  supabase/functions/
                </code>{" "}
                נפרס מיידית. השתמש בכפתור "בדוק הכל" לוודא שהפונקציות פעילות.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {groupedFunctions.map(({ category, meta, functions: catFns }) => {
              ...
              const catActive = catFns.filter(
                (f) => f.status === "active",
              ).length;
              const catError = catFns.filter(
                (f) => f.status === "error",
              ).length;
          ...
          >
            <AlertTriangle
              className={cn("h-5 w-5 mt-0.5 shrink-0", goldIcon)}
            />
            <div className="text-sm">
              ...
              <p className="text-yellow-600/80 dark:text-yellow-400/80 mt-1">
                בדיקת תקינות שולחת{" "}
                <code className="bg-yellow-500/20 px-1.5 py-0.5 rounded text-xs">
                  health_check
                </code>{" "}
                לכל פונקציה. חלק מהפונקציות עלולות להחזיר שגיאה אם הן מצפות
                לפרמטרים ספציפיים — זה תקין. השתמש ב"הפעל ידנית" כדי לשלוח בקשה
                מותאמת אישית.
              </p>
            </div>
          </div>
      ...
      <Card
        className={cn(
          goldBg,
          "border-2 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]",
        )}
      >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-indigo-700 dark:text-indigo-300">
                איך לפרוס Edge Function חדשה?
              </span>
            </div>
            <div className="text-sm text-indigo-600/80 dark:text-indigo-400/80 space-y-2">
              <div className="flex items-start gap-2">
                <Badge className="bg-indigo-500/20 text-indigo-600 border-indigo-500/30 text-xs mt-0.5 shrink-0">
                  1
                </Badge>
                <span>
                  צור תיקייה בשם הפונקציה תחת{" "}
                  <code className="bg-indigo-500/20 px-1.5 py-0.5 rounded text-xs">
                    supabase/functions/שם-הפונקציה/
                  </code>
                </span>
              </div>
              <div className="flex items-start gap-2">
                ...
                <span>
                  ...
                  בתוך התיקייה עם הקוד של הפונקציה
                </span>
              </div>
              <div className="flex items-start gap-2">
                <Badge className="bg-indigo-500/20 text-indigo-600 border-indigo-500/30 text-xs mt-0.5 shrink-0">
                ...
                <span>
                  שמור דרך <strong>Lovable</strong> — הפריסה תקרה{" "}
            ...
            {uploadedFiles.length > 0 && (
              <div className="space-y-2">
                {uploadedFiles.map((f, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between text-sm bg-background rounded-lg p-3 border"
                  >
                    <div className="flex items-center gap-2">
                ...
                >
                  <div className="text-sm space-y-2">
                    ...
                    <ol className="text-green-600/80 dark:text-green-400/80 space-y-1 mr-6 list-decimal">
                      ...
                      <li>
                        פתח את Lovable ונווט ל{" "}
                        <code className="bg-green-500/20 px-1 py-0.5 rounded text-xs">
                          supabase/functions/שם-הפונקציה/index.ts
                        </code>
                      </li>
          ...
          >
            ...
            <div className="text-sm">
              ...
              <p className="text-purple-600/80 dark:text-purple-400/80 mt-1">
                הדרך הכי פשוטה: פתח את <strong>Lovable</strong>, ערוך או צור
                קובץ בתיקיית
                <code className="bg-purple-500/20 px-1.5 py-0.5 rounded text-xs mx-1">
                  supabase/functions/
                </code>
          ...
          <div className="space-y-3">
            ...
            {testResults.length === 0 && (
              <div className="text-center text-muted-foreground py-8">
                אין תוצאות בדיקה עדיין. לחץ "בדוק הכל" כדי לבדוק את כל
                הפונקציות.
              </div>
            )}
          </div>
