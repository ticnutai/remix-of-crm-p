...
function extractItemsFromText(text: string): ExtractedItem[] {
  ...
  for (const line of lines) {
    ...
    if (
      trimmedLine.includes("הצעת מחיר") ||
    ...
    if (priceMatches && priceMatches.length >= 1) {
      const priceStr = priceMatches[priceMatches.length - 1].replace(
        /[₪,\s]/g,
        "",
      );
...
function extractStages(text: string): TemplateStage[] {
  ...
  for (const line of lines) {
    ...
    const isHeader =
      sectionPatterns.some((p) => p.test(trimmed)) ||
...
function detectCategory(text: string): string {
  ...
  if (lowerText.includes("היתר") || lowerText.includes("רישוי"))
    return "development";
  if (
    lowerText.includes("תוספת") ||
    lowerText.includes("הרחבה") ||
    lowerText.includes("בניה") ||
    lowerText.includes("בנייה")
  )
    return "construction";
  if (lowerText.includes("שיפוץ")) return "marketing";
  if (lowerText.includes("פנים") || lowerText.includes("עיצוב"))
    return "design";
  if (lowerText.includes("פיקוח") || lowerText.includes("ייעוץ"))
    return "consulting";
 ...
 */
export async function importDocumentToTemplate(
  file: File,
  ...
  try {
    ...
    const styledHtml = htmlContent
      ? wrapHtmlWithStyling(htmlContent, title)
      : null;
