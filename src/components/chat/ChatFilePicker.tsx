...
function FileIcon({
  ...
  if (t.startsWith("image/"))
    return <FileImage className={cn("h-5 w-5 text-blue-500", className)} />;
  if (t.startsWith("video/"))
    return <FileVideo className={cn("h-5 w-5 text-purple-500", className)} />;
  if (t.startsWith("audio/"))
    return <FileAudio className={cn("h-5 w-5 text-pink-500", className)} />;
  if (t.includes("pdf"))
    return <FileText className={cn("h-5 w-5 text-red-500", className)} />;
  if (t.includes("word") || t.includes("document"))
    return <FileText className={cn("h-5 w-5 text-blue-700", className)} />;
  if (t.includes("sheet") || t.includes("excel"))
    return <FileText className={cn("h-5 w-5 text-green-600", className)} />;
  if (t.includes("zip") || t.includes("rar") || t.includes("archive"))
    return <FileArchive className={cn("h-5 w-5 text-yellow-600", className)} />;
  if (
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            <p className="font-medium text-sm">
              {dragging ? "שחרר לכאן!" : "גרור קבצים או לחץ לבחירה"}
            </p>
...
function GoogleDriveTab({ onPick }: { onPick: (f: PickedFile) => void }) {
  const { listFiles, searchFiles, files, folders, isLoading } =
    useGoogleDrive();
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          size="icon"
          className="h-8 w-8"
          onClick={() => listFiles(currentFolder)}
        >
              ...
              >
                ...
                <div className="flex-1 min-w-0 text-right">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {file.modifiedTime &&
                      ` • ${format(new Date(file.modifiedTime), "dd/MM/yy", { locale: he })}`}
                  </p>
                </div>
