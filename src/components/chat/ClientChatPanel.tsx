...
export function ClientChatPanel({ clientId, clientName, className }: ClientChatPanelProps) {
  const { user, profile } = useAuth();
  const {
    conversations, messages, activeConversation,
    selectConversation, sendMessage, createConversation, sendingMessage, loading,
  } = useChat();
  ...
  useEffect(() => {
    if (clientConv && (!activeConversation || activeConversation.id !== clientConv.id)) {
  ...
  if (!clientConv && !activeConversation) {
    return (
      <div className={cn('border rounded-2xl p-6 flex flex-col items-center gap-3 text-center bg-muted/20', className)}>
        <div className="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center">
          <MessageCircle className="h-6 w-6 text-blue-600" />
        </div>
        <div>
          <h4 className="font-semibold text-sm">פתח שיחה עם {clientName}</h4>
          <p className="text-xs text-muted-foreground mt-0.5">ניהול שיחות וידוא מול הלקוח</p>
        </div>
        <Button onClick={handleStartConversation} disabled={starting} className="rounded-xl gap-2">
  ...
  return (
    <div className={cn('border rounded-2xl flex flex-col overflow-hidden bg-background', className)} dir="rtl">
      {/* Header */}
      <div className="flex items-center gap-2.5 px-4 py-2.5 border-b bg-muted/20">
        ...
        {(clientConv?.unread_count || 0) > 0 && (
          <Badge className="bg-destructive rounded-full text-[10px]">{clientConv!.unread_count}</Badge>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {messages.filter(m => !m.is_deleted).map(msg => {
              ...
              return (
                <div key={msg.id} className={cn('flex gap-1.5', isOwn ? 'flex-row-reverse' : 'flex-row')}>
                  {!isOwn && (
                    <Avatar className="h-6 w-6 mt-0.5 shrink-0">
                      <AvatarFallback className="text-[9px] bg-blue-100">{(msg.sender_name || 'L').slice(0, 2)}</AvatarFallback>
                    </Avatar>
                  )}
                  <div className={cn(
                    ...
                    <div className={cn(
                      'text-[9px] mt-0.5',
                      isOwn ? 'text-primary-foreground/60 text-left' : 'text-muted-foreground'
      ...
      <div className="p-2 border-t flex gap-2">
        ...
        />
        <Button size="icon" className="h-9 w-9 rounded-xl shrink-0" onClick={handleSend} disabled={!input.trim() || sendingMessage}>
