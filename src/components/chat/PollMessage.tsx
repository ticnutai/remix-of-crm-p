...
export function PollMessage({
  ...
  return (
    <div className="min-w-[200px] max-w-[260px] space-y-2">
      ...
      {poll.allow_multiple && (
        <p className={cn("text-[10px]", subColor)}>ניתן לבחור מספר תשובות</p>
      )}
        ...
        {((poll.options || []) as PollOption[]).map((opt) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={cn(
                  "absolute inset-y-0 right-0 rounded-lg transition-all",
                  barFill,
                  voted && "opacity-90",
                )}
                style={{ width: `${pct}%` }}
              ...
              <div className="relative flex items-center justify-between px-2.5 py-1.5">
                <div className="flex items-center gap-1.5">
                  {voted ? (
                    <CheckCircle2
                      className={cn("h-3 w-3 shrink-0", textColor)}
                    />
                  ) : (
                    <div
                      className={cn(
                ...
                <span className={cn("text-[10px] shrink-0", subColor)}>
                  {pct}%
                </span>
              </div>
            </button>
      ...
      <p className={cn("text-[10px]", subColor)}>
        {totalVotes} {totalVotes === 1 ? "מצביע" : "מצביעים"}
      </p>
    </div>
  );
}
