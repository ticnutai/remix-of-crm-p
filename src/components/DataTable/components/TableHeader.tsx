...
import { ColumnDef, SortState, FilterState } from '../types';
...
export function TableHeader<T>({
  ...
  const getSortIcon = (columnId: string) => {
    ...
    if (sort.direction === 'asc') return <ArrowUp className="h-4 w-4 text-secondary" />;
  ...
  const headerClasses = cn(
    'sticky top-0 z-10',
    {
      'bg-table-header text-table-header-foreground': variant === 'default' || variant === 'navy',
      'bg-secondary/20 text-foreground border-b-2 border-border-gold': variant === 'gold',
      'bg-muted text-foreground': variant === 'minimal',
    }
  );
          ...
          const headerContent = (
            <th
              ...
              className={cn(
                ...
                column.sortable !== false && 'cursor-pointer hover:bg-primary/10',
            ...
            >
              <div className="flex items-center gap-2 justify-end">
                ...
                {isFilterable && onFilterChange && (
                  <div className={cn(
                    "transition-opacity",
                    getColumnFilter(accessorKey) ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                  )}>
                    <ColumnFilter
                      column={column}
                      currentFilter={getColumnFilter(accessorKey)}
                      onFilterChange={(filter) => onFilterChange(filter, accessorKey)}
                      data={data}
                      variant={variant}
                    />
                  </div>
                ...
                {column.sortable !== false && (
                  <span className={cn(
                    ...
                    sorts.find(s => s.columnId === accessorKey) ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                  )}>
                ...
                <div className="flex flex-col items-end">
                  {column.headerEditable && typeof column.header === 'string' ? (
                    <EditableHeader
                      value={column.header}
