...
import { Check, CheckCheck, ListPlus, Loader2, Plus, Search, SlidersHorizontal } from "lucide-react";
...
export function BulkClientFieldsDialog({
  ...
  const { user } = useAuth();
  const { definitions, updateField } = useClientCustomFields({ enabled: open });
    ...
    try {
      ...
      if (field.custom) {
        ...
        for (const id of [...added, ...removed]) {
          ...
          const { error } = await supabase.from("clients").update({ custom_data: customData }).eq("id", id);
          if (error) throw error;
        }
      ...
      <DialogContent className="max-w-6xl h-[82vh]" contentClassName="p-0 overflow-hidden block">
        ...
        <div className="grid h-[calc(82vh-104px)] grid-cols-[240px_260px_1fr] divide-x divide-x-reverse" dir="rtl">
          <ScrollArea className="p-4">
            <p className="mb-3 text-sm font-semibold">1. בחירת שדה</p>
            <div className="space-y-1">
              {fields.map((item) => (
                <button key={item.key} onClick={() => { setFieldKey(item.key); setValue(""); }} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-muted", fieldKey === item.key && "bg-primary text-primary-foreground hover:bg-primary") }>
                  <span className="block font-medium">{item.label}</span><span className="block text-[11px] opacity-70">{item.custom ? "שדה מותאם" : item.section}</span>
                </button>
              ))}
            </div>
          </ScrollArea>
          <div className="flex min-h-0 flex-col p-4">
            ...
            {!field ? <p className="text-sm text-muted-foreground">בחר שדה מהרשימה</p> : <>
              ...
              <ScrollArea className="min-h-0 flex-1"><div className="space-y-1 pr-1">{options.map((option) => <button key={option} onClick={() => setValue(option)} className={cn("flex w-full items-center justify-between rounded-lg border px-3 py-2 text-right text-sm hover:bg-muted", value === option && "border-primary bg-primary/10 font-semibold")}><span>{option}</span>{value === option && <Check className="h-4 w-4 text-primary" />}</button>)}{options.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">אין עדיין אפשרויות. הוסף את הראשונה למעלה.</p>}</div></ScrollArea>
            </>}
          </div>
          <div className="flex min-h-0 flex-col p-4">
            ...
            {value && <><div className="relative mb-3"><Search className="absolute right-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} className="pr-9" placeholder="חיפוש לקוח..." /></div><ScrollArea className="min-h-0 flex-1 rounded-xl border"><div className="divide-y">{visibleClients.map((client) => { const checked = selected.has(client.id); return <button key={client.id} onClick={() => setSelected((prev) => { const next = new Set(prev); checked ? next.delete(client.id) : next.add(client.id); return next; })} className="flex w-full items-center gap-3 px-4 py-3 text-right hover:bg-muted/70"><span className={cn("flex h-5 w-5 items-center justify-center rounded border", checked && "border-primary bg-primary text-primary-foreground")}>{checked && <Check className="h-3.5 w-3.5" />}</span><span className="flex-1 font-medium">{client.name}</span><span className="text-xs text-muted-foreground">{valueOf(client, field!) || "ללא ערך"}</span></button>; })}</div></ScrollArea></>}
