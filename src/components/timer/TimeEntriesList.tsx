...
export function TimeEntriesList() {
  const { todayEntries, refreshEntries } = useTimer();
  const { theme: timerTheme } = useTimerTheme();
  ...
  if (todayEntries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-[hsl(45,80%,50%)]/20 to-[hsl(45,80%,40%)]/10 flex items-center justify-center mb-4 border-2 border-[hsl(45,80%,50%)]/30">
          <Sparkles className="h-8 w-8 text-[hsl(45,70%,55%)]" />
        </div>
      ...
      {todayEntries.map(entry => (
        <div
          ...
          className={cn(
            ...
            "bg-[hsl(220,60%,20%)] hover:bg-[hsl(220,60%,24%)]",
            "border-2 border-[hsl(45,80%,50%)]/20 hover:border-[hsl(45,80%,50%)]/40",
            "hover:shadow-lg hover:shadow-[hsl(45,80%,50%)]/10",
            entry.is_running && "border-[hsl(45,80%,55%)] bg-[hsl(220,60%,22%)] shadow-md shadow-[hsl(45,80%,50%)]/20"
          )}
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              {editingId === entry.id ? (
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    className="h-8 text-sm bg-white/10 border-[hsl(45,80%,50%)]/50 text-white"
                  ...
                  <Button
                    ...
                    onClick={() => saveDescription(entry.id)}
                  >
                ...
                <button
                  onClick={() => startEditing(entry.id, entry.description || '')}
                  className={cn(
                    "font-semibold truncate text-right w-full flex items-center gap-2 group/desc",
                    "hover:opacity-80 transition-opacity cursor-pointer"
                  )}
                  style={{
                    color: entry.description 
                      ? (timerTheme.entryTextColor || 'rgba(255,255,255,0.9)') 
                      : (timerTheme.labelsColor || 'hsl(45,40%,55%)'),
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {entry.project_id && projects[entry.project_id] && (
                  <Badge className="text-xs bg-[hsl(220,60%,30%)] text-[hsl(45,70%,65%)] border border-[hsl(45,80%,50%)]/30 hover:bg-[hsl(220,60%,35%)]">
                    <Briefcase className="h-3 w-3 ml-1" />
                    {projects[entry.project_id]}
                  </Badge>
                )}
                {entry.client_id && clients[entry.client_id] && (
                  <Badge className="text-xs bg-[hsl(220,60%,30%)] text-[hsl(45,70%,65%)] border border-[hsl(45,80%,50%)]/30 hover:bg-[hsl(220,60%,35%)]">
                ...
                {entry.is_billable && (
                  <Badge className="text-xs bg-gradient-to-r from-[hsl(45,80%,50%)] to-[hsl(45,90%,45%)] text-[hsl(220,60%,15%)] font-semibold">
                    לחיוב
                  </Badge>
                )}
              </div>
            ...
            <div className="flex flex-col items-end gap-2">
              <div className={cn(
                ...
                entry.is_running 
                  ? "text-[hsl(45,90%,60%)] drop-shadow-[0_0_8px_rgba(200,160,60,0.5)]" 
                  : "text-[hsl(45,70%,60%)]"
              )}>
              ...
              {!entry.is_running && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 rounded-lg text-[hsl(220,30%,50%)] hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all"
