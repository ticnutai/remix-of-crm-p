...
function FloatingTimerContent() {
  ...
  const [currentSizePreset, setCurrentSizePreset] = useState<
    "small" | "medium" | "large"
  >("medium");
  ...
  const onResizeStart = useCallback(
    (e: React.MouseEvent | React.TouchEvent, dir: string) => {
      e.preventDefault();
      e.stopPropagation();
      setIsResizing(true);
      setResizeDir(dir);
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (resizeDir?.includes("e"))
        newW = Math.max(250, Math.min(600, resizeStartRef.current.w + dx));
      if (resizeDir?.includes("w"))
        newW = Math.max(250, Math.min(600, resizeStartRef.current.w - dx));
      if (resizeDir?.includes("s"))
        newH = Math.max(350, Math.min(800, resizeStartRef.current.h + dy));
  ...
  const cycleSizePreset = useCallback(() => {
    const presets: Array<"small" | "medium" | "large"> = [
      "small",
      "medium",
      "large",
    ];
    ...
    toast.success(
      `גודל: ${nextPreset === "small" ? "קטן" : nextPreset === "medium" ? "בינוני" : "גדול"}`,
    );
  }, [currentSizePreset]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
            ...
            <span className="relative flex flex-col items-center justify-center h-full w-full">
              ...
              {isMinimized && timerState.elapsed > 0 && (
                <>
                  <Timer
                    className={cn(
                      "h-4 w-4 mb-0.5 transition-colors duration-300",
                      timerState.isRunning
                        ? "text-[hsl(45,85%,60%)]"
                        : "text-[hsl(45,70%,55%)]",
                    )}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: timerState.isRunning
                        ? timerTheme.controlButtonsActiveColor ||
                          "hsl(45,80%,60%)"
      ...
      >
        ...
        <div
          className="absolute top-2 right-2 w-6 h-6 border-t-2 border-r-2 rounded-tr-xl opacity-60 pointer-events-none"
          style={{
            borderColor: timerTheme.accentColor,
          }}
        />
        <div
          className="absolute top-2 left-2 w-6 h-6 border-t-2 border-l-2 rounded-tl-xl opacity-60 pointer-events-none"
          style={{
            borderColor: timerTheme.accentColor,
          }}
        />
        <div
          className="absolute bottom-2 right-2 w-6 h-6 border-b-2 border-r-2 rounded-br-xl opacity-60 pointer-events-none"
          style={{
            borderColor: timerTheme.accentColor,
          }}
        />
        <div
          className="absolute bottom-2 left-2 w-6 h-6 border-b-2 border-l-2 rounded-bl-xl opacity-60 pointer-events-none"
          style={{
            borderColor: timerTheme.accentColor,
          }}
        />
        ...
        <div
          className="relative px-6 py-5 border-b-2 group rounded-t-3xl"
          style={{
            backgroundColor: timerTheme.backgroundColor,
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        "h-14 w-14 rounded-2xl flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-95 border-2 disabled:cursor-not-allowed shadow-lg",
                        timerState.isRunning
                          ? "bg-white/20 border-white/40 hover:bg-white/30"
                          : "bg-white border-[#d8ac27] hover:bg-gray-50",
                      )}
                    >
                      {timerState.isRunning ? (
                        <Pause className="h-6 w-6 text-white" />
                      ) : (
                        <Play className="h-6 w-6 text-[#d8ac27]" />
                      )}
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <QuickInputSection
                type="title"
                onSelect={(title) =>
                  setStopDescription((prev) =>
                    prev ? `${prev}, ${title}` : title,
                  )
                }
                selectedValue={stopDescription}
              />
              <Input
                id="stop-description"
            ...
            <div className="space-y-2">
              ...
              <QuickInputSection
                type="note"
                onSelect={(note) =>
                  setStopNotes((prev) => (prev ? `${prev}, ${note}` : note))
                }
                selectedValue={stopNotes}
              />
              <Textarea
                id="stop-notes"
                value={stopNotes}
