...
const SIZE_PRESETS = {
  small: { width: 260, height: 400 },
  medium: { width: 320, height: 500 },
  large: { width: 380, height: 600 },
};

function FloatingTimerContent() {
  const { timerState, todayTotal, weekTotal, stopTimer, resumeTimer, startTimer, pauseTimer, resetTimer, saveEntry } = useTimer();
  ...
  const [position, setPosition] = useState(() => {
    ...
    return { x: 32, y: typeof window !== 'undefined' ? window.innerHeight - 100 : 600 };
  });
  ...
  const onResizeStart = useCallback((e: React.MouseEvent | React.TouchEvent, dir: string) => {
    ...
    resizeStartRef.current = { x: clientX, y: clientY, w: popoverSize.width, h: popoverSize.height };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <button
            ...
            className={cn(
              ...
              "hover:shadow-[0_0_30px_rgba(180,140,50,0.5),inset_0_1px_0_rgba(255,255,255,0.15)]",
              "hover:scale-105 active:scale-95",
              "focus:outline-none focus:ring-2 focus:ring-[hsl(45,80%,50%)]/50 focus:ring-offset-2",
              timerState.isRunning && [
                "border-[hsl(45,90%,55%)]",
                "shadow-[0_0_25px_rgba(200,160,60,0.5),inset_0_1px_0_rgba(255,255,255,0.15)]",
              ],
              isDragging && "cursor-grabbing scale-110 shadow-[0_0_40px_rgba(180,140,50,0.7)]",
            ...
            <span className="relative flex items-center justify-center h-full w-full">
              <Timer 
                className={cn(
                  ...
                  timerState.isRunning ? "text-[hsl(45,85%,60%)]" : "text-[hsl(45,70%,55%)] group-hover:text-[hsl(45,85%,65%)]"
                )} 
                strokeWidth={1.8} 
              />
            </span>
          ...
          {(timerState.isRunning || timerState.elapsed > 0) && (
            <div 
              className={cn(
                ...
                "hover:shadow-[0_0_30px_rgba(180,140,50,0.4)] transition-all duration-300"
              )}
              ...
              <div className="flex items-center gap-1.5">
                ...
                >
                  {timerState.isRunning ? (
                    ...
                    <Play className="h-4 w-4 fill-current mr-[-1px]" />
                  )}
                </button>
              ...
              <div className="flex flex-col items-start text-right">
                <span 
                  className={cn(
                    ...
                    timerState.isRunning && "drop-shadow-[0_0_8px_rgba(212,175,55,0.4)]"
                  )}
                  style={{
      ...
      >
        ...
        />
        <div 
          className="absolute -top-2 -left-2 w-8 h-8 cursor-nw-resize z-50 hover:bg-[hsl(45,80%,50%)]/30 active:bg-[hsl(45,80%,50%)]/50 rounded-full transition-colors touch-none" 
          onMouseDown={(e) => onResizeStart(e, 'nw')} 
          onTouchStart={(e) => onResizeStart(e, 'nw')} 
        />
        <div 
          className="absolute -bottom-2 -right-2 w-8 h-8 cursor-se-resize z-50 hover:bg-[hsl(45,80%,50%)]/30 active:bg-[hsl(45,80%,50%)]/50 rounded-full transition-colors touch-none" 
          onMouseDown={(e) => onResizeStart(e, 'se')} 
          onTouchStart={(e) => onResizeStart(e, 'se')} 
        />
        <div 
          className="absolute -bottom-2 -left-2 w-8 h-8 cursor-sw-resize z-50 hover:bg-[hsl(45,80%,50%)]/30 active:bg-[hsl(45,80%,50%)]/50 rounded-full transition-colors touch-none" 
          onMouseDown={(e) => onResizeStart(e, 'sw')} 
          onTouchStart={(e) => onResizeStart(e, 'sw')} 
        />
        {/* Edges - larger for touch */}
        <div 
          className="absolute -top-2 left-8 right-8 h-4 cursor-n-resize z-40 touch-none" 
          onMouseDown={(e) => onResizeStart(e, 'n')} 
          onTouchStart={(e) => onResizeStart(e, 'n')} 
        />
        <div 
          className="absolute -bottom-2 left-8 right-8 h-4 cursor-s-resize z-40 touch-none" 
          onMouseDown={(e) => onResizeStart(e, 's')} 
          onTouchStart={(e) => onResizeStart(e, 's')} 
        />
        <div 
          className="absolute -left-2 top-8 bottom-8 w-4 cursor-w-resize z-40 touch-none" 
          onMouseDown={(e) => onResizeStart(e, 'w')} 
          onTouchStart={(e) => onResizeStart(e, 'w')} 
        />
        <div 
          className="absolute -right-2 top-8 bottom-8 w-4 cursor-e-resize z-40 touch-none" 
        ...
        <div 
          className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-16 h-2 rounded-full bg-[hsl(45,80%,50%)]/60 cursor-s-resize z-50 touch-none active:bg-[hsl(45,80%,50%)]/80" 
          onMouseDown={(e) => onResizeStart(e, 's')} 
          onTouchStart={(e) => onResizeStart(e, 's')} 
        />
        <div 
          className="absolute -right-4 top-1/2 -translate-y-1/2 w-2 h-16 rounded-full bg-[hsl(45,80%,50%)]/60 cursor-e-resize z-50 touch-none active:bg-[hsl(45,80%,50%)]/80" 
          onMouseDown={(e) => onResizeStart(e, 'e')} 
          onTouchStart={(e) => onResizeStart(e, 'e')} 
        />
        <div 
          className="absolute -left-4 top-1/2 -translate-y-1/2 w-2 h-16 rounded-full bg-[hsl(45,80%,50%)]/60 cursor-w-resize z-50 touch-none active:bg-[hsl(45,80%,50%)]/80" 
          onMouseDown={(e) => onResizeStart(e, 'w')} 
          onTouchStart={(e) => onResizeStart(e, 'w')} 
        />
        
        {/* Decorative corner ornaments */}
        <div className="absolute top-2 right-2 w-6 h-6 border-t-2 border-r-2 rounded-tr-xl opacity-60 pointer-events-none" style={{ borderColor: timerTheme.accentColor }} />
        <div className="absolute top-2 left-2 w-6 h-6 border-t-2 border-l-2 rounded-tl-xl opacity-60 pointer-events-none" style={{ borderColor: timerTheme.accentColor }} />
        <div className="absolute bottom-2 right-2 w-6 h-6 border-b-2 border-r-2 rounded-br-xl opacity-60 pointer-events-none" style={{ borderColor: timerTheme.accentColor }} />
        <div className="absolute bottom-2 left-2 w-6 h-6 border-b-2 border-l-2 rounded-bl-xl opacity-60 pointer-events-none" style={{ borderColor: timerTheme.accentColor }} />
        
        {/* Timer Display Header - Main Timer at Top with Rounded Corners */}
        <div className="relative px-6 py-5 border-b-2 group rounded-t-3xl" style={{ backgroundColor: timerTheme.backgroundColor, borderColor: `${timerTheme.borderColor}66` }}>
          {/* Abstract decorative elements */}
          <div className="absolute top-3 left-16 opacity-20">
            <Sparkles className="h-5 w-5" style={{ color: timerTheme.accentColor }} />
          </div>
          <div className="absolute bottom-3 right-24 opacity-15">
            <Gem className="h-4 w-4" style={{ color: timerTheme.accentColor }} />
          </div>
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div 
              className={cn(
                "font-light tracking-widest text-center transition-all duration-300",
                timerState.isRunning && "drop-shadow-[0_0_20px_rgba(212,175,55,0.6)]"
              )}
              style={{
                fontSize: `${timerTheme.timerDisplayFontSize || 48}px`,
                fontFamily: `"${timerTheme.timerFontFamily || 'JetBrains Mono'}", monospace`,
                color: timerState.isRunning 
                  ? (timerTheme.accentColor || 'hsl(45, 85%, 55%)') 
                  : (timerTheme.timerDigitsColor || 'hsl(0, 0%, 100%)'),
            ...
            <div className="flex items-center gap-4">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={async () => { await stopTimer(); }}
                      disabled={!timerState.isRunning && timerState.elapsed === 0}
                      className={cn(
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        timerState.isRunning 
                          ...
                          : "bg-[hsl(45,80%,50%)]/30 border-[hsl(45,80%,50%)]/50 hover:bg-[hsl(45,80%,50%)]/40"
                      )}
                    >
                      {timerState.isRunning ? (
                        <Pause className="h-6 w-6 text-white" />
                      ) : (
                        <Play className="h-6 w-6 text-[hsl(45,85%,65%)]" />
                      )}
                    </button>
                  </TooltipTrigger>
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={async () => { await saveEntry(); toast.success('הזמן נשמר בהצלחה'); }}
                      disabled={timerState.elapsed === 0}
                      className={cn(
                        "h-11 w-11 rounded-xl flex items-center justify-center transition-all duration-200",
                        "hover:scale-110 active:scale-95 border",
          ...
          <div className="p-5 space-y-5 max-w-full overflow-hidden">
            ...
            <div className="relative py-3">
              <div className="absolute inset-0 flex items-center px-4">
                <div className="w-full border-t-2" style={{ borderColor: `${timerTheme.borderColor}4D` }} />
              </div>
