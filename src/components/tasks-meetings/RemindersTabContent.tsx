...
import { format, isPast, isFuture, isToday, addMinutes, addHours } from 'date-fns';
...
const priorityConfig = {
  high: { label: 'גבוהה', icon: ArrowUp, color: 'text-red-500', badge: 'bg-red-100 text-red-700 border-red-200' },
  medium: { label: 'בינונית', icon: ArrowRight, color: 'text-yellow-500', badge: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
  low: { label: 'נמוכה', icon: ArrowDown, color: 'text-green-500', badge: 'bg-green-100 text-green-700 border-green-200' },
};

export function RemindersTabContent() {
  const { reminders, loading, deleteReminder, dismissReminder, updateReminder } = useReminders();
  ...
  const pendingReminders = filteredReminders.filter(r => !r.is_sent && !r.is_dismissed && isFuture(new Date(r.remind_at)));
  const todayReminders = filteredReminders.filter(r => isToday(new Date(r.remind_at)));
  const overdueReminders = filteredReminders.filter(r => !r.is_sent && !r.is_dismissed && isPast(new Date(r.remind_at)));
  const pastReminders = filteredReminders.filter(r => r.is_sent || r.is_dismissed);
  ...
  const handleSnooze = async (reminder: Reminder, minutes: number) => {
    ...
    try {
      await supabase
        .from('reminders')
        .update({ remind_at: newTime.toISOString(), is_sent: false, is_dismissed: false })
        .eq('id', reminder.id);
      toast.success(`תזכורת נדחתה ל-${format(newTime, 'HH:mm', { locale: he })}`);
  ...
  const getStatusBadge = (reminder: Reminder) => {
    ...
    if (reminder.is_sent) return <Badge className="bg-green-600 text-white">נשלחה</Badge>;
    if (isPast(new Date(reminder.remind_at))) return <Badge variant="destructive">באיחור</Badge>;
    if (isToday(new Date(reminder.remind_at))) return <Badge className="bg-[hsl(45,80%,45%)] text-white">היום</Badge>;
          ...
          {!reminder.is_dismissed && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8" title="נודניק">
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleSnooze(reminder, 60 * 24)}>
                  <Clock className="h-4 w-4 ml-2" />
                  מחר
                </DropdownMenuItem>
              </DropdownMenuContent>
    ...
    <Table>
      ...
      <TableBody>
        {items.length === 0 ? (
          ...
          items.map((reminder) => <ReminderRow key={reminder.id} reminder={reminder} />)
        )}
      </TableBody>
    </Table>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-[hsl(45,80%,45%)]">{todayReminders.length}</p>
      ...
      {/* Sub-tabs */}
      <Tabs defaultValue={overdueReminders.length > 0 ? 'overdue' : 'pending'} dir="rtl">
        <TabsList>
          {overdueReminders.length > 0 && (
