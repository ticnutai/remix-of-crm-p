...
export function CustomTableTab({
  table,
  onDeleteTable,
  canManage,
}: CustomTableTabProps) {
  ...
  const [newColumnType, setNewColumnType] = useState<
    "text" | "number" | "boolean" | "date" | "select" | "data_type"
  >("text");
    ...
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (fileExtension === "csv") {
          ...
          toast({
            title: "יובא בהצלחה",
            description: `${importedCount} שורות יובאו מקובץ CSV`,
          });
          ...
          toast({
            title: "יובא בהצלחה",
            description: `${importedCount} שורות יובאו מקובץ Excel`,
          });
        }
      } catch (error) {
        toast({
      ...
      switch (col.type) {
        ...
        case "select":
          return {
            ...baseColumn,
            editType: "select",
            editOptions: (col.options || []).map((opt) => ({
              value: opt,
              label: opt,
            })),
            cell: (value: any) =>
  ...
  const handleCellEdit = useCallback(
    async (row: CustomTableData, columnId: string, newValue: any) => {
      saveToHistory();
      const updatedData = {
        ...row.data,
        [columnId]: newValue,
  ...
  const handleQuickAddRows = useCallback(
  ...
  const handleBulkColumnsAdded = useCallback(
    async (bulkColumns: any[], groupName?: string) => {
      ...
      toast({
        title: "עמודות נוספו",
        description: `${bulkColumns.length} עמודות נוספו${groupName ? ` לקבוצה "${groupName}"` : ""} בהצלחה`,
      });
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            {data.length} רשומות | {table.columns.length} עמודות |{" "}
            {table.description ||
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canManage && (
            <div className="flex items-center gap-1 border-l pl-2">
              ...
              >
                <TableRowsSplit className="h-4 w-4" />+ שורות
              </Button>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <Columns className="h-4 w-4" />+ עמודות
              </Button>
              <Button
                variant="outline"
                size="sm"
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                        {[
                          ...
                          },
                          {
                            ...
                            label: "בחירה",
                            icon: List,
                            description: "רשימת אפשרויות",
                          },
                          {
                            type: "data_type",
                            label: "סוג נתון",
                            icon: Link2,
                            description: "קישור לנתון אחר (לקוח, עובד, פרויקט)",
                          },
                          ...
                          return (
                            ...
                            >
                              <Icon
                                className={`h-4 w-4 mb-1 ${isSelected ? "text-primary" : "text-muted-foreground"}`}
                              />
                    ...
                    {newColumnType === "data_type" && (
                      <div className="grid gap-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          ניתן ליצור סוגי נתונים חדשים דרך ניהול סוגי נתונים
                          בסרגל הצד
                        </p>
                      </div>
                    )}
                      ...
                      newColumnType !== "boolean" && (
                        <div className="grid gap-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setNewColumnDefault(e.target.value)
                            }
                          />
                        </div>
                      )}
