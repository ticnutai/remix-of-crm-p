...
import {
  ...
  createImportPhases,
  ImportPhase,
} from "@/components/backup/ImportProgressPanel";
import {
  normalizeExternalBackup,
  getSupportedFormats,
  NormalizedBackup,
} from "@/utils/backupNormalizer";
...
// Auto Backup Settings sub-component
function AutoBackupSettings() {
  ...
  const config = autoBackupStatus?.config;
  ...
  const selectedDays = config?.daysOfWeek ?? [0, 1, 2, 3, 4, 5, 6];
  const timesPerDay = config?.timesPerDay ?? 1;
  const customTimes = config?.customTimes ?? [config?.time || "02:00"];

  const toggleDay = (day: number) => {
    const current = [...selectedDays];
    const idx = current.indexOf(day);
    if (idx >= 0) {
      if (current.length <= 1) return; // חייב לפחות יום אחד
      current.splice(idx, 1);
    } else {
      current.push(day);
      current.sort();
    }
    updateConfig({ daysOfWeek: current });
  };
  ...
  const updateCustomTime = (index: number, value: string) => {
    ...
    newTimes.sort();
    updateConfig({ customTimes: newTimes, time: newTimes[0] });
  };

  const handleTimesPerDayChange = (count: number) => {
    const clamped = Math.max(1, Math.min(4, count));
    const baseTime = config?.time || "02:00";
    // יצירת שעות אוטומטיות מרווחות
    const newTimes: string[] = [];
    const gap = Math.floor(24 / clamped);
    const [baseH] = baseTime.split(":").map(Number);
    for (let i = 0; i < clamped; i++) {
      const h = (baseH + gap * i) % 24;
      newTimes.push(`${String(h).padStart(2, "0")}:00`);
    }
    updateConfig({ timesPerDay: clamped, customTimes: newTimes });
  };
  ...
  const handleTrigger = async () => {
    ...
    try {
      await triggerBackup();
      toast({ title: "✅ גיבוי אוטומטי הושלם" });
    } catch (e: any) {
      toast({
        title: "שגיאה בגיבוי",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between py-2">
        <div>
          <p className="font-medium">הפעלת גיבוי אוטומטי</p>
          <p className="text-sm text-muted-foreground">
            {enabled ? "פעיל" : "כבוי"} • נשמר לענן ולמחשב
          </p>
        </div>
        <Switch
          checked={enabled}
      ...
      {enabled && (
        <>
          ...
          {config?.frequency !== "hourly" && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label>כמה פעמים ביום</Label>
                <Select
                  value={String(timesPerDay)}
                  onValueChange={(v) => handleTimesPerDayChange(parseInt(v))}
                >
          ...
          {/* Save locations */}
          <div className="flex items-center justify-between">
            <Label>שמור לענן</Label>
            <Switch
              checked={config?.saveToCloud ?? true}
          ...
          <div className="flex items-center justify-between">
            ...
            <Input
              ...
              onChange={(e) =>
                updateConfig({ maxBackups: parseInt(e.target.value) || 7 })
              }
              className="w-[80px]"
            />
          </div>
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>
              ...
              {autoBackupStatus?.lastBackup
                ? formatDistanceToNow(new Date(autoBackupStatus.lastBackup), {
                    locale: he,
                    addSuffix: true,
                  })
                : "לא בוצע עדיין"}
...
export default function Backups() {
  ...
  const [isExternalImportDialogOpen, setIsExternalImportDialogOpen] =
    useState(false);
  ...
  const [externalBackupData, setExternalBackupData] =
    useState<ArchFlowBackup | null>(null);
  ...
  const [isCloudRestoreDialogOpen, setIsCloudRestoreDialogOpen] =
    useState(false);
    ...
    const metadataSheet = XLSX.utils.json_to_sheet([
      {
        ...
        סה_כ_רשומות: Object.values(data).reduce(
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      const tableCount = Object.keys(data).filter(
        (k) => data[k]?.length > 0,
      ).length;
  ...
  const handleExternalFileSelect = async (
    ...
    try {
      ...
      if (!result.success || !result.data) {
        ...
        const errorDetails = [
          ...
          result.detectedKeys?.length
            ? `מפתחות שנמצאו: ${result.detectedKeys.slice(0, 5).join(", ")}${result.detectedKeys.length > 5 ? "..." : ""}`
            : "",
          "",
          "פורמטים נתמכים:",
  ...
  const handleExternalImport = async () => {
    ...
    try {
      ...
      const dataToImport = {
        ...
        ClientStageTask:
          rawData.ClientStageTask || rawData.client_stage_tasks || [],
        StageTemplate: rawData.StageTemplate || rawData.stage_templates || [],
        StageTemplateStage:
          rawData.StageTemplateStage || rawData.stage_template_stages || [],
        StageTemplateTask:
          rawData.StageTemplateTask || rawData.stage_template_tasks || [],
      };
      ...
      for (const client of existingClients || []) {
        if (client.name)
          clientNameMap.set(client.name.toLowerCase().trim(), client.id);
        if (client.name_clean)
          clientNameMap.set(client.name_clean.toLowerCase().trim(), client.id);
        if (client.email)
          clientEmailMap.set(client.email.toLowerCase().trim(), client.id);
      }

      for (const project of existingProjects || []) {
        if (project.name)
          projectNameMap.set(project.name.toLowerCase().trim(), project.id);
      }
      ...
      if (importOptions.clients && dataToImport.Client) {
        ...
        for (let i = 0; i < clients.length; i += BATCH_SIZE) {
          ...
          setProgressMessage(
            `מייבא לקוחות (${Math.min(i + BATCH_SIZE, clients.length)}/${clients.length})...`,
          );
          ...
          for (const client of batch) {
            ...
            if (
              client.custom_data &&
              Object.keys(client.custom_data).length > 0
            ) {
            ...
            if (inserted && !error) {
              ...
              if (normalizedEmail)
                clientEmailMap.set(normalizedEmail, inserted.id);
      ...
      if (importOptions.projects && dataToImport.Project) {
        ...
        for (let i = 0; i < projects.length; i += BATCH_SIZE) {
          ...
          setProgressMessage(
            `מייבא פרויקטים (${Math.min(i + BATCH_SIZE, projects.length)}/${projects.length})...`,
          );

          for (const project of batch) {
            // Check for duplicates using in-memory map
            const normalizedProjectName = project.name?.toLowerCase().trim();
            const existingProjectId = normalizedProjectName
              ? projectNameMap.get(normalizedProjectName)
              : null;
            ...
            const mappedClientId = project.client_id
              ? clientIdMap.get(project.client_id) || null
              : null;
            ...
            if (inserted && !error) {
              ...
              if (normalizedProjectName)
                projectNameMap.set(normalizedProjectName, inserted.id);
      ...
      if (importOptions.time_entries && dataToImport.TimeLog) {
        ...
        for (let i = 0; i < timeLogs.length; i++) {
          ...
          if (i % 100 === 0) {
            ...
            setProgressMessage(
              `מעבד רישומי זמן (${i + 1}/${timeLogs.length})...`,
            );
          }

          // Try ID mapping first, then fallback to name lookup
          let mappedClientId = timeLog.client_id
            ? clientIdMap.get(timeLog.client_id) || null
            : null;

          // Fallback: lookup by client_name if ID mapping failed
          if (!mappedClientId && timeLog.client_name) {
            const normalizedClientName = timeLog.client_name
              .toLowerCase()
              .trim();
          ...
          const endTime = new Date(
            startTime.getTime() + (timeLog.duration_seconds || 0) * 1000,
          );
          const description =
            [timeLog.title, timeLog.notes].filter(Boolean).join(" - ") || null;
        ...
        for (let i = 0; i < timeEntriesToInsert.length; i += TIME_BATCH_SIZE) {
          ...
          setProgress(
            60 + ((i + batch.length) / timeEntriesToInsert.length) * 20,
          );
          setProgressMessage(
            `מייבא רישומי זמן (${Math.min(i + TIME_BATCH_SIZE, timeEntriesToInsert.length)}/${timeEntriesToInsert.length})...`,
          );
      ...
      if (importOptions.tasks && dataToImport.Task) {
        ...
        for (let i = 0; i < tasks.length; i += BATCH_SIZE) {
          ...
          setProgressMessage(
            `מייבא משימות (${Math.min(i + BATCH_SIZE, tasks.length)}/${tasks.length})...`,
          );

          for (const task of batch) {
            const mappedClientId = task.client_id
              ? clientIdMap.get(task.client_id) || null
              : null;
      ...
      if (importOptions.meetings && dataToImport.Meeting) {
        ...
        for (let i = 0; i < meetings.length; i += BATCH_SIZE) {
          ...
          setProgressMessage(
            `מייבא פגישות (${Math.min(i + BATCH_SIZE, meetings.length)}/${meetings.length})...`,
          );

          for (const meeting of batch) {
            const mappedClientId = meeting.client_id
              ? clientIdMap.get(meeting.client_id) || null
              : null;
            ...
            const meetingEndTime =
              ...
              (meetingStartTime
                ? new Date(
                    new Date(meetingStartTime).getTime() + 3600000,
                  ).toISOString()
                : new Date().toISOString());
      ...
      const importedItems = [];
      if (stats.clients.imported > 0)
        importedItems.push(`${stats.clients.imported} לקוחות`);
      if (stats.projects.imported > 0)
        importedItems.push(`${stats.projects.imported} פרויקטים`);
      if (stats.tasks.imported > 0)
        importedItems.push(`${stats.tasks.imported} משימות`);
      if (stats.meetings.imported > 0)
        importedItems.push(`${stats.meetings.imported} פגישות`);
      if (stats.time_entries.imported > 0)
        importedItems.push(`${stats.time_entries.imported} רישומי זמן`);
      if (stats.client_stages.imported > 0)
        importedItems.push(`${stats.client_stages.imported} שלבי לקוחות`);
      if (stats.stage_templates.imported > 0)
        importedItems.push(`${stats.stage_templates.imported} תבניות שלבים`);
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "אירעה שגיאה במהלך ייבוא הנתונים",
        variant: "destructive",
      });
    }
  };
  ...
  const handleAutoImportFromFile = async (resumeFrom?: ImportProgress) => {
    ...
    try {
      ...
      // Import Projects
      if (
        (startPhase === "clients" || startPhase === "projects") &&
      ...
      const autoTimeLogs = backupData.data.TimeLog || backupData.data.timeLogs;
      if (autoTimeLogs) {
        const timeLogs = autoTimeLogs;
        stats.time_entries.total = timeLogs.length;
        setProgressMessage(`מכין רישומי זמן לייבוא...`);

        // Build clientNameMap from existing DB clients for name-based fallback
        const { data: allClients } = await supabase
          .from("clients")
          .select("id, name");
        const autoClientNameMap = new Map<string, string>();
        (allClients || []).forEach((c) => {
          if (c.name) autoClientNameMap.set(c.name.toLowerCase().trim(), c.id);
        });
        ...
        for (let i = 0; i < timeLogs.length; i++) {
          ...
          // Map client_id - try ID mapping first, then fallback to name lookup
          let mappedClientId = timeLog.client_id
            ? clientIdMap.get(timeLog.client_id) || null
            : null;
          ...
          if (!mappedClientId && timeLog.client_name) {
            const normalizedClientName = timeLog.client_name
              ...
              .trim();
            mappedClientId =
              autoClientNameMap.get(normalizedClientName) || null;
          }
          ...
          const endTime = new Date(
            startTime.getTime() + timeLog.duration_seconds * 1000,
          );
          const description =
            [timeLog.title, timeLog.notes].filter(Boolean).join(" - ") || null;
        ...
        for (let i = 0; i < timeEntriesToInsert.length; i += BATCH_SIZE) {
          ...
          setProgress(
            80 + ((i + batch.length) / timeEntriesToInsert.length) * 20,
          );
          setProgressMessage(
            `מייבא רישומי זמן (${Math.min(i + BATCH_SIZE, timeEntriesToInsert.length)}/${timeEntriesToInsert.length})...`,
          );
  ...
  const repairJsonInputRef = useRef<HTMLInputElement>(null);

  // Repair time entries from uploaded JSON backup file
  const handleRepairFromJsonFile = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (!file || !user) return;
    ...
    try {
      ...
      // Extract time logs from various backup formats
      const backupTimeLogs: ArchFlowTimeLog[] =
        backupData.data?.TimeLog ||
        backupData.data?.timeLogs ||
        backupData.TimeLog ||
        backupData.timeLogs ||
        [];

      if (backupTimeLogs.length === 0) {
        toast({
          title: "לא נמצאו לוגי זמן",
          description: "הקובץ לא מכיל נתוני לוגי זמן",
          variant: "destructive",
        });
      ...
      clients.forEach((c) => {
        if (c.name) clientNameLookup.set(c.name.trim().toLowerCase(), c.id);
      });

      // Fetch ALL time entries (not just unlinked - we can re-link even mislinked ones)
      const { data: allEntries } = await supabase
        .from("time_entries")
        .select("id, description, start_time, client_id")
        .eq("user_id", user.id);

      if (!allEntries || allEntries.length === 0) {
        toast({
          title: "אין רשומות",
          description: "אין רשומות זמן במערכת",
          variant: "destructive",
        });
        setIsFixingUnlinked(false);
        return;
      ...
      // Build lookup from backup: key(date + description) -> client_name
      const backupLookup = new Map<string, string>();
      for (const log of backupTimeLogs) {
        ...
        backupLookup.set(`${dateStr}|${description}`, log.client_name);
        if (log.title) {
          backupLookup.set(`${dateStr}|${log.title}`, log.client_name);
        }
      }

      // Only fix entries with null client_id
      const unlinkedEntries = allEntries.filter((e) => !e.client_id);
      ...
      for (const entry of unlinkedEntries) {
        ...
        // Try full match
        const clientName = backupLookup.get(`${dateStr}|${desc}`);
        if (clientName) {
          matchedClientId =
            clientNameLookup.get(clientName.trim().toLowerCase()) || null;
        }

        // Try partial match
        if (!matchedClientId && desc) {
          const descPart = desc.split(" - ")[0].trim();
          const partialClientName = backupLookup.get(`${dateStr}|${descPart}`);
          if (partialClientName) {
            matchedClientId =
              clientNameLookup.get(partialClientName.trim().toLowerCase()) ||
              null;
          }
        }

        // Fallback: client name in description
        if (!matchedClientId) {
      ...
      toast({
        title: "תיקון מקובץ גיבוי הושלם",
        description: `חוברו ${fixed} רשומות מתוך ${unlinkedEntries.length}, ${notFound} לא נמצא התאמה`,
      });
    } catch (error) {
      console.error("Repair from JSON failed:", error);
      toast({
        title: "שגיאה בתיקון",
        description: "בדוק שהקובץ בפורמט JSON תקין",
        variant: "destructive",
      });
    }

    setIsFixingUnlinked(false);
    if (repairJsonInputRef.current) repairJsonInputRef.current.value = "";
  };

  // Import time logs from simple CSV format (לקוח,תאריך,שעה,כותרת...)
  const handleTimeLogsCSVImport = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (!file || !user) return;
    ...
    try {
      ...
      setProgress(20);
      setProgressMessage(
        `נוצרו ${clientsCreated} לקוחות חדשים. מייבא רישומי זמן...`,
      );

      // Fetch existing time entries for duplicate checking
      const { data: existingEntries } = await (
      ...
      const stats: ImportStats = {
        clients: {
          total: clientNamesFromCSV.length,
          imported: clientsCreated,
          skipped: clientNamesFromCSV.length - clientsCreated,
        },
        projects: { total: 0, imported: 0, skipped: 0 },
      ...
      for (let i = 0; i < timeEntriesToInsert.length; i += BATCH_SIZE) {
        const batch = timeEntriesToInsert.slice(i, i + BATCH_SIZE);
        setProgress(
          70 + ((i + batch.length) / timeEntriesToInsert.length) * 30,
        );
        setProgressMessage(
          `מייבא (${Math.min(i + BATCH_SIZE, timeEntriesToInsert.length)}/${timeEntriesToInsert.length})...`,
        );
  ...
  const handleCSVImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < timeLogs.length; i++) {
        ...
        const description =
          [log.title, log.notes].filter(Boolean).join(" - ") || null;
  ...
  // Fix existing unlinked time entries by matching client names from backup JSON data
  const handleFixUnlinkedTimeEntries = async () => {
    ...
    try {
      // Step 1: Fetch all clients from DB
      const { data: clients } = await supabase
        .from("clients")
        .select("id, name");
      ...
      // Step 2: Fetch unlinked time entries (no client_id)
      ...
      // Step 3: Build client name lookup
      const clientNameLookup = new Map<string, string>();
      clients.forEach((c) => {
        if (c.name) clientNameLookup.set(c.name.trim().toLowerCase(), c.id);
      });

      // Step 4: Try to load backup JSON to get client_name mapping
      let backupTimeLogs: ArchFlowTimeLog[] = [];
      try {
        const response = await fetch("/backups/archflow-backup.json");
        if (response.ok) {
          const backupData = (await response.json()) as ArchFlowBackup;
          backupTimeLogs =
            backupData.data?.TimeLog || backupData.data?.timeLogs || [];
        }
      } catch {
        console.log("No backup JSON found, will try description matching only");
      }

      // Also try backup_to_import.json if no backup found
      if (backupTimeLogs.length === 0) {
        try {
          const response = await fetch("/backup_to_import.json");
          if (response.ok) {
          ...
          console.log("No backup_to_import.json found either");
        }
      }

      // Build a lookup from backup: key(date + description) -> client_name
      const backupLookup = new Map<string, string>();
      for (const log of backupTimeLogs) {
        if (!log.client_name) continue;
        const logDate = new Date(log.log_date);
        ...
        backupLookup.set(key, log.client_name);
        // Also add simplified key without notes in case description was truncated
        if (log.title) {
          const simpleKey = `${dateStr}|${log.title}`;
          if (!backupLookup.has(simpleKey)) {
            backupLookup.set(simpleKey, log.client_name);
          }
        }
      }

      console.log(
        `[FixUnlinked] Found ${unlinkedEntries.length} unlinked entries, ${backupTimeLogs.length} backup logs, ${clients.length} clients`,
      );
      ...
      for (const entry of unlinkedEntries) {
        ...
        // Method 1: Match via backup JSON data (most reliable)
        if (!matchedClientId && backupLookup.size > 0) {
          const key = `${dateStr}|${desc}`;
          const clientName = backupLookup.get(key);
          if (clientName) {
            matchedClientId =
              clientNameLookup.get(clientName.trim().toLowerCase()) || null;
          }
          // Try partial key match (description might have minor differences)
          if (!matchedClientId && desc) {
            const descPart = desc.split(" - ")[0].trim();
            const partialKey = `${dateStr}|${descPart}`;
            const partialClientName = backupLookup.get(partialKey);
            if (partialClientName) {
              matchedClientId =
                clientNameLookup.get(partialClientName.trim().toLowerCase()) ||
                null;
        ...
        // Method 2: Client name appears in description (fallback)
        if (!matchedClientId) {
          const descLower = desc.toLowerCase();
          for (const client of clients) {
            const normalizedName = client.name.trim().toLowerCase();
      ...
      <div className="p-6 space-y-6 animate-fade-in">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>צור גיבוי חדש או ייבא גיבוי קיים</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-4">
                  ...
                  {status === "creating" ? (
                    <div className="py-6 space-y-4">
                      ...
                      <p className="text-center text-muted-foreground">
                        {progressMessage ||
                          `מגבה נתונים... ${Math.round(progress)}%`}
                      </p>
                    </div>
                  ) : status === "success" ? (
                      ...
                      <div className="space-y-6 py-4 max-h-[60vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {Object.entries(backupTopics).map(
                              ([key, checked]) => (
                                <div
                                  key={key}
                        ...
                        <div className="space-y-3">
                          ...
                          <p className="text-xs text-muted-foreground">
                            JSON מומלץ לשחזור מלא • Excel מומלץ לצפייה ועריכה
                            ידנית
                          </p>
                        </div>
              ...
              {/* Fix unlinked time entries */}
              <Button
                variant="outline"
                className="border-orange-500 text-orange-600 hover:bg-orange-50"
                onClick={handleFixUnlinkedTimeEntries}
                disabled={isFixingUnlinked}
              >
                {isFixingUnlinked ? (
                  <Loader2 className="h-4 w-4 ml-2 animate-spin" />
                ) : (
                ...
                {isFixingUnlinked
                  ? `מתקן... (${fixProgress.fixed}/${fixProgress.total})`
                  : "תקן שיוך לוגי זמן ללקוחות"}
              </Button>

              {/* Fix from uploaded JSON backup */}
              <Button
                variant="outline"
                className="border-orange-500 text-orange-600 hover:bg-orange-50"
                onClick={() => repairJsonInputRef.current?.click()}
                disabled={isFixingUnlinked}
              >
                <FileJson className="h-4 w-4 ml-2" />
                תקן שיוך מקובץ גיבוי (JSON)
              </Button>
              <input
                type="file"
                ref={repairJsonInputRef}
                onChange={handleRepairFromJsonFile}
                accept=".json"
                className="hidden"
              />
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent dir="rtl">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        פעולה זו תמחק את כל הגיבויים השמורים לצמיתות. לא ניתן
                        לשחזר אותם.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  גיבויים אוטומטיים לפי ההגדרות שלך
                  {cloudBackups.length > 0 &&
                    ` • ${cloudBackups.length} גיבויים זמינים`}
                </CardDescription>
              </div>
              <div className="flex gap-2">
                <Button
                ...
                >
                  <RefreshCw
                    className={cn(
                      "h-4 w-4",
                      loadingCloudBackups && "animate-spin",
                    )}
                  />
                </Button>
                <Button
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent dir="rtl">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  האם למחוק את הגיבוי "{backup.name}"? פעולה זו
                                  לא ניתנת לביטול.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
        ...
        >
          <DialogContent dir="rtl">
            <DialogHeader>
              ...
              <DialogDescription>
                השחזור יוריד את קובץ הגיבוי לבדיקה. שחזור מלא לבסיס הנתונים דורש
                אישור נוסף.
              </DialogDescription>
            </DialogHeader>
                  ...
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm">
                        ...
                        <p className="text-muted-foreground">
                          הקובץ יורד לבדיקה. לשחזור מלא יש לייבא את הנתונים
                          ידנית או ליצור קשר עם מנהל המערכת.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <Dialog
          open={isExternalImportDialogOpen}
          onOpenChange={(open) => {
              ...
              <div className="py-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 mt-4">
                  ...
                  <div className="p-4 bg-green-500/10 rounded-lg text-center">
                    ...
                    <p className="text-2xl font-bold">
                      {importStats.time_entries.imported}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      רישומי זמן יובאו
                    </p>
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <span>
                          {format(
                            new Date(externalBackupData.generated_at),
                            "dd/MM/yyyy HH:mm",
                            { locale: he },
                          )}
                        </span>
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                time_entries: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-timelogs"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                meetings: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-meetings"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                quotes: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-quotes"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <Badge variant="secondary">
                          {
                            (
                              ((
                                externalBackupData.data as Record<
                                  string,
                                  unknown
                                >
                              ).Invoice as unknown[]) || []
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                custom_spreadsheets: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-spreadsheets"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                team_members: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-team-members"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                client_feedback: checked as boolean,
                              }))
                            }
                          />
                          <Label
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                internal_messages: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-messages"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                chat_conversations: checked as boolean,
                              }))
                            }
                          />
                          <Label
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                user_preferences: checked as boolean,
                              }))
                            }
                          />
                          <Label
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setImportOptions((prev) => ({
                                ...prev,
                                access_control: checked as boolean,
                              }))
                            }
                          />
                          <Label
                            htmlFor="import-access"
                            className="flex items-center gap-2 cursor-pointer"
                          >
                  ...
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm">
                        <p className="font-medium text-yellow-600">שים לב</p>
                        <p className="text-muted-foreground">
                          הייבוא יוסיף את הנתונים לבסיס הנתונים הקיים. פעולה זו
                          לא ניתנת לביטול. מומלץ ליצור גיבוי לפני הייבוא.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <Dialog
          open={isCloudRestoreDialogOpen}
          onOpenChange={setIsCloudRestoreDialogOpen}
        >
          <DialogContent dir="rtl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    {selectedCloudBackup.created_at && (
                      <div>
                        ...
                        <span>
                          {format(
                            new Date(selectedCloudBackup.created_at),
                            "dd/MM/yyyy HH:mm",
                            { locale: he },
                          )}
                        </span>
                ...
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm">
                      <p className="font-medium text-yellow-600">שים לב</p>
                      <p className="text-muted-foreground">
                        השחזור ייבא את כל הנתונים מהגיבוי. רשומות כפולות ידולגו
                        אוטומטית.
                      </p>
                    </div>
                  </div>
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setIsCloudRestoreDialogOpen(false)}
