...
import { Loader2, CreditCard, CheckCircle2, Clock, AlertCircle, Receipt, TrendingUp, Banknote, ArrowLeft } from 'lucide-react';
...
export default function ClientPayments() {
  ...
  const summary = useMemo(() => {
    const totalAmount = stages.reduce((sum, s) => sum + (s.amount_with_vat ?? s.amount), 0);
    const paidAmount = stages.filter(s => s.is_paid).reduce((sum, s) => sum + (s.paid_amount ?? s.amount_with_vat ?? s.amount), 0);
    ...
    return { totalAmount, paidAmount, remainingAmount: totalAmount - paidAmount, paidCount, pendingCount, progressPercent };
  }, [stages]);
        ...
        {stages.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">שלבי התשלום יופיעו כאן כשהצוות יוסיף אותם</p>
            </CardContent>
          </Card>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-sm font-bold mt-0.5">{formatCurrency(summary.totalAmount)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-sm font-bold mt-0.5 text-green-600 dark:text-green-400">{formatCurrency(summary.paidAmount)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-sm font-bold mt-0.5 text-orange-600 dark:text-orange-400">{formatCurrency(summary.remainingAmount)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-sm font-bold mt-0.5">{summary.progressPercent}%</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">{summary.paidCount}/{stages.length} שלבים</span>
                </div>
            ...
            <div className="space-y-3">
              <h2 className="text-sm font-semibold text-muted-foreground">פירוט שלבי תשלום</h2>
              {stages.map((stage) => (
                <Card
                  key={stage.id}
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <Badge variant={stage.is_paid ? 'default' : 'outline'} className={stage.is_paid ? 'bg-green-600 hover:bg-green-700' : ''}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                      <div className="bg-muted/50 rounded-lg p-2">
                        <span className="text-muted-foreground">סכום (ללא מע"מ)</span>
                        <p className="font-semibold mt-0.5">{formatCurrency(stage.amount)}</p>
                      </div>
                      <div className="bg-muted/50 rounded-lg p-2">
                        <span className="text-muted-foreground">סכום כולל מע"מ</span>
                        <p className="font-semibold mt-0.5">{formatCurrency(stage.amount_with_vat ?? stage.amount)}</p>
                      </div>
                    </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-dashed text-xs space-y-1">
                        {stage.paid_date && (
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{format(new Date(stage.paid_date), 'dd/MM/yyyy', { locale: he })}</span>
                          </div>
                        )}
                        {stage.payment_method && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">אמצעי תשלום</span>
                            <span className="font-medium">{paymentMethodLabels[stage.payment_method] || stage.payment_method}</span>
                          </div>
                        )}
                        {stage.paid_amount != null && stage.paid_amount > 0 && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">סכום ששולם</span>
                            <span className="font-medium text-green-600 dark:text-green-400">{formatCurrency(stage.paid_amount)}</span>
                          </div>
                        )}
                      </div>
                    )}
